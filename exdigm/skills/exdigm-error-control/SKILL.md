...
`OperationalError` 행의 존재는 과거 실패 증거이지 현재 장애 판정이 아니다. 현재 배포 코드의 공식 경로와 저장 상태를 기준으로 다시 구분한다.

- 공식 경로가 예외 없이 종료되고 `missing_career`처럼 필수 원문 정보 부족으로 저장을 거부했거나 `hold_non_resume`처럼 비이력서로 분류했다면 Candidate·Resume가 없는 것이 기대 결과다. 이를 추출 오류나 미처리 건으로 보고하지 않는다.
- 과거 코드 오류를 수정한 뒤 같은 원본이 정상 정책 결과로 끝났고 부분 저장·처리 중 상태·대기열 잔존·새 운영 오류가 없으면 처리 완료로 체크포인트만 갱신한다. 사용자의 판단이 필요하지 않으면 최종 응답은 `NO_REPLY`다.
- 원문에 필수 정보가 실제로 있는데 정책 거부가 발생했거나, 진입 경로에 따라 판정이 다르거나, 예외·부분 저장·재시도 잔량이 남은 경우에만 새 분류·처리 결함으로 조사한다.
- 보고가 필요할 때도 “이력서가 생성되지 않음” 자체를 오류로 쓰지 않는다. 원래 시스템 결함과 현재 정상 정책 결과를 분리해 설명한다.
...
실제 오류를 보고할 때는 오류 하나당 아래 3줄 순서를 정확히 사용한다. 여러 오류는 빈 줄 하나로 구분한다.

```text
YYYY-MM-DD HH:MM KST 간단요약
내용
ID: 오류 ID
```

- 첫 줄에는 발생 일시와 한 문장 이내의 간단요약만 쓴다.
- 둘째 줄에는 실패 내용·업무 영향·확인된 원인·수정 또는 배포 상태를 필요한 만큼 간단히 쓴다.
- 셋째 줄에는 오류 ID만 쓴다.
- 오류별 제목·불릿·필드 순서를 추가하거나 바꾸지 않는다. 사용자의 판단이 필요할 때만 모든 오류 블록 뒤에 별도 한 문장으로 다음 행동을 덧붙인다.
