...
## 운영 반영 완료 — 2026-09-16 23:52 KST

- 주인님의 다른 변경 포함 배포 승인으로 `8084524b`의 제품 코드·검사 3파일을 그대로 최종 통합 커밋 `de20cf82c962a3acc140e669a67cd350886e9eb2`에 넣어 공식 prod로 배포했다. 원본 별도 detached worktree는 보존한다.
- 실제 명단 3변형과 정상 이력서 before/after 응답의 공식 저장 경로 5개를 통합 코드에서 다시 통과했다. 최종 합동 검사 733 passed/4 skipped와 보호 216 passed, 실제 이미지 문서 계약을 통과했다.
- 운영 앱·updater가 최종 판본을 사용한다. 원본 명단과 정상 대조 FileData 전체 행 해시, Candidate/Resume 연결, 큐 상태가 배포 전후 같다. 과거 명단 실패를 재실행하거나 비이력서로 소급 정정하지 않았다.
- 아래 미배포 기록은 당시 이력이다. 현재 배포·운영 보존 정본은 [최종 통합 기록](auto-posting-other-sites-20260916.md)이다. 운영 명단 제외 사유의 소급 변경은 여전히 별도 요청 범위다.
