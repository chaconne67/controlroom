# 혼합 언어 이력서 추출 오류 60ead5b9 — 운영 반영 승인, 샘플 확인·후보자 등록 제외

## 2026-09-21 저녁 후속 승인과 현재 선행 조건

- 주인님은 이 대화에서 “고친 내용을 실제 서비스에 적용하고, 실패한 이력서 한 건을 다시 처리”하는 설명에 `오케이`로 답했다. 이후 실행 담당 확인에도 **“이번 한 건은 여기서 직접 완료”**로 답했다. 승인 범위는 기존 지시문 3줄과 FileData `d7ba0f95-d074-411d-b182-8609836b5fb5` 한 건이다. 같은 승인을 다시 요구하지 않는다. Telegram 승인이나 다른 작업의 승인으로 기록하지 않는다.
- 최신 운영 기준은 `8d5b2963f737d7a69aa2f707e1a23641375e1849`다. 이 기준에서 해당 추출 파일은 기존 `efbab7d3`와 동일하고, 보관 커밋 `d69f5012`의 3줄 diff는 `git apply --check`를 통과했다. 수정본 자체는 아직 미배포다.
- 원래 FileData의 저장 없음·후보자 연결 없음·원문 SHA-256은 그대로다. 단건 목록도 해당 Drive ID 한 건을 가리킨다.
- 운영 저장 전 공식 `identify_candidate(..., strict=True)`를 조회 전용으로 실행하자 `phone_email_mismatch`가 확인됐다. 새 원문의 이름·이메일은 기존 전화번호 일치 후보자와 다르다. 두 파일의 실제 원문에도 같은 전화번호가 있으므로 AI 추출 오류나 현재 DB의 단순 표시 오류로 고칠 근거가 없다. 기존 후보자 번호는 `3112d480-5152-4ecf-8340-16d781afc488`, 그 원본 FileData는 `309ef1c0-56e5-41b8-b0ce-60ca833fc175`이다. 연락처 원문은 비공개 자료에만 둔다.
- 주인님이 **“이 파일은 샘플 파일이다. 샘플 파일인데 기존 후보자의 파일을 사용해서 하다보니 연락처를 안 바꾸고 해서 그렇다. 이건 실제 후보자 파일이 아니다.”**라고 확인했다. 이 답변으로 연락처 충돌의 원천은 확인됐다. 원문 추출이나 후보자 매칭 정책을 고칠 사안이 아니다.
- 최신 완료 조건은 검증된 지시문 3줄의 운영 반영과 샘플이라는 사실의 오류 기록이다. 이 파일의 운영 재처리·실제 Candidate/Resume 생성은 하지 않는다. 이미 완료한 원본/변형/기존 성공 자료의 격리 추출 검증은 유지하며, 기존 후보자 `3112d480-5152-4ecf-8340-16d781afc488`와 관련 원본·이력서는 보존한다. 이름이나 전화번호 모양만으로 샘플을 자동 판정하는 새 규칙은 만들지 않는다.

아래 내용은 낮 시간에 완료한 조사·수정·검증 기록이다. 아래의 승인·배포 표현은 당시 상태이며 위 승인과 실행 담당 결정이 이를 대체한다.
...
운영 DB는 `scripts/debug_workspace.sh shell-readonly`의 `exdigm_debug_ro`, `transaction_read_only=on`, `Asia/Seoul`을 확인하고 조회했다. 원래 FileData는 09:33:38 KST 이후 변경되지 않았고 `db_saved_at=null`, `candidate_id=null`, Resume 0건, `needs_resume_processing=false`, `retryable=false`이다. 원래 이력서의 복구는 아직 완료되지 않았다.

수정 커밋은 `d69f50126ee76caacdb7ee7fe6cd2ab62a7c65fe`, 운영 기준은 `efbab7d33ac0c276ae1afe88cda6dbf5b7ee96a2`이다. 후자는 운영에 반영된 실패 응답 보관 기능이다. 이번 중복 판단 지시문 변경과 해당 파일 재처리는 아직 운영에 적용하지 않았다.
...
- 최소 구현 게이트: 2단계에서 멈춤. 기존 `mixed_language_source_prompt`, `GenerateResumeCtype.create_canonical_source`, `scripts/debug_workspace.sh test`를 재사용한다. 새 생성 경로·검사기·의존성을 만들지 않았다. 조사용 응답 관찰은 원래 입력·호출 인수·응답을 바꾸지 않는다.
...
1. 정확한 커밋 `d69f50126ee76caacdb7ee7fe6cd2ab62a7c65fe`와 현재 운영 기준을 다시 대조한다. 이 문서는 새 배포 권한을 부여하지 않는다.
2. 단건 준비 목록은 `load_succeeded_rows(..., force=True, limit=1)`로 정확히 해당 Drive ID 한 건만 선택됨을 확인했다. 164행 추출 원문과 149행 정리 텍스트의 사본·해시를 보존했다. **실행 직전** 최신 FileData·원문과 연락처로 연결될 기존 후보자/이력서/관련 정보를 다시 백업한다. 현재 `production-baseline.json`은 조사 시점 사본이므로 최신 백업을 대신하지 않는다.
3. 현행 공식 단건 진입점은 `data_extraction.services.candidate_update_steps.realtime_txt_to_db` 모듈이다. 위 단건 전용 자료 루트를 `EXDIGM_WORK_ROOT`로 지정하고 `--source manifest --force --workers 1 --limit 1`을 사용한다. 파이프라인 잠금·운영 실행 환경을 확인한다. 예전 문서의 `extract_text_to_db` 관리 명령은 현행 코드에 없다.
4. 구조화 JSON, Resume 생성, Candidate 연결, 저장 완료 시각과 중복 저장 여부를 실제 DB에서 확인한다. 실패하면 먼저 부분 저장과 실패 응답을 확인하고 무조건 재시도하지 않는다. 같은 오류의 후속 이력에 실제 결과를 남긴다.
5. 완료는 원래 운영 결과와 재발 경로의 공식 검증으로 판정한다. 실패 응답 보관 기능의 배포·검사 성공으로 대신하지 않는다.
