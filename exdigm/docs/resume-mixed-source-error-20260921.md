# 혼합 언어 이력서 추출 오류 60ead5b9 — 운영 반영 승인, 실행 준비와 연락처 확인
...
- 주인님께 이 파일이 익명 예시인지, 기존 후보자와 같은 사람인지, 별도 실제 후보자인지와 후자의 확인 가능한 연락처를 질문했다. 후보자를 자동 병합하거나 전화번호를 임의 삭제·교체하거나 신원 충돌 검사를 약화하지 않는다. 코드 배포와 이 추가 데이터 확인은 별도로 진행한다.
