...
작성일: 2026-09-18 KST. 개정: 2026-09-19 KST. 상태: **v2 코드 구현·개발 검증 및 승인 대기 중 다음 사건 처리 보완 완료. 운영 배포와 제한 권한 연결·무인 실행 활성화 대기. 직접 기록·분류/이력은 Exdigm debug에, 단건 실행기와 쥬디 연결 정본은 Controlroom에 보관한다. 쥬디의 기존 로컬 조회는 적용했다. 실제 자동 수리·운영 전달의 종단 검증은 활성화 단계에서 수행한다. 재개는 10절을 따른다.**
...
**현재 승인 입력의 구현 경계:** `approve_deploy`는 요청이며 허가가 아니다. 현재 main 단건 실행기는 `investigate/repair`만 확보하고 배포를 수행하지 않는다. 쥬디의 대화 답장을 승인 결정으로 DB에 저장하는 기능과 그 결정을 받아 자동 배포하는 연결도 아직 없다. 현재는 이 조정실 등 실제 지시를 받은 에이전트가 대상 오류·요청 개정·커밋을 확인하고, 주인님의 명시적 승인과 실행 결과를 기존 결과 명령의 처리 이력에 남기는 절차다. 단순 메시지 전달, 요청 열람, 시간 경과, 다른 건에 대한 승인을 허가로 해석하지 않는다. 향후 쥬디 답장 연결을 구현할 때는 승인 원문/출처·대상 오류·요청 개정·커밋의 결합과 제한된 실행 권한을 실제로 검증해야 하며 알림 조회 성공을 승인 연결 완료로 보고하지 않는다.
...
승인된 배포를 맡은 조정실 에이전트는 같은 debug 잠금·예약을 확보하고 DB의 요청 개정, `base_commit`, `commit`, `repair_ref`를 대조한다. 실제 Git 참조가 승인된 커밋을 가리키는지 확인한 뒤 그 커밋을 detached HEAD로 선택해 공식 검사·배포 경로를 사용한다. 다른 요청의 미승인 커밋을 함께 배포하지 않는다. 그동안 다른 배포로 운영 기준이 바뀌었다면 원본 수정본은 보존하고 새 운영 기준에 반영·검증한 결과와 차이를 기록한다. 새 커밋을 예전 SHA의 승인으로 배포하지 않고 갱신한 배포 요청의 승인을 받는다. 배포와 작업 공간 정리 후 소유를 반환한다.
