...
**최신 결정 — 2026-09-21:** 사건·트랙 단위의 현행 역할과 상태 흐름은 [운영 오류 사건 파이프라인 v3](operational-error-event-pipeline-20260921.md)가 정본이다. 샘은 보고·실제 사용자 결정·전달 영수증을 DB에 기록하는 통신만 담당한다. 조사·수정·배포·운영 결과 검증은 main 서버의 분리된 Codex 작업자와 systemd timer가 담당한다. 이 문서에 남은 “샘이 수정·배포한다”는 표현은 이전 결정과 설치 이력이며 현행 실행 권한으로 사용하지 않는다. v3 구현·검증·운영 전환 상태는 새 정본 문서에서 갱신한다.
