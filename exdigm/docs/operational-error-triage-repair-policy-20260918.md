...
작성일: 2026-09-18 KST. 개정: 2026-09-19 KST. 상태: **주인님의 Telegram 직접 「초기 배포 승인」으로 제품 커밋 `3141ce76bef167c59700951f65edc306f01b7968`과 additive migration `0070`의 운영 배포·사후 검증을 완료했고 기존 오류 원문·이력 59건 보존을 확인했다. 사실 증거 기록 보완 커밋 `e0606504859b10298456387ef17780b5ce6d4c68`도 운영 배포·사후 검증을 완료했다. 실제 배포는 로컬 조정실 Codex가 먼저 실행했고 샘은 중복 실행을 중단한 뒤 사후 검증과 보고만 수행했다. 이후 샘이 전달한 배포 요청은 샘만 실행하도록 Controlroom `54a865bd7a5b43b9817523cbe243f464fff96ec7`과 main 설치본을 갱신했다. main의 단건 실행기는 Controlroom `4cc8910ea7e39473d037f1c81292d91a2545d82d`로 동기화했지만 제한 설정과 systemd unit/timer가 없어 자동 실행은 아직 꺼져 있다. 최신 근거와 재개는 아래 「사실 기록과 실행기 검증 경계 보완」을 따른다.**
...
Codex 종료 코드 0이나 JSON 형식 통과를 실제 수리 성공으로 취급하지 않는다. 연결 코드는 보고된 SHA가 실제 clean debug HEAD인지 먼저 대조하고, 그 커밋에서 공식 `scripts/debug_workspace.sh test`와 `check`를 직접 실행한다. 두 명령의 종료 상태 0과 출력을 실행별 영수증에 저장한 경우에만 배포 승인 요청을 기록한다. 확인할 수 없으면 미완료로 남긴다. 원인 미확인을 승인 요청으로 떠넘기지 않고 부족한 증거를 구체적으로 적는다.
...
`approve_deploy`는 실제 debug HEAD=전체 커밋 SHA, 시작 HEAD와 다름, clean 상태를 확인해야 한다. Codex가 출력한 검사 문장이나 CLI 명령 문자열은 실행 성공 증거로 사용하지 않는다. 같은 단건 실행기가 공식 `scripts/debug_workspace.sh test`와 `check`를 직접 실행하고, 종료 상태 0·명령 이름·출력 파일·대상 커밋을 원자적인 `official-verification.json`에 기록한다. 중단 후 같은 커밋의 완료된 검사는 영수증과 출력 파일을 대조해 재사용하고, 실패한 검사는 성공으로 기록하지 않는다. 실패 원인·검사 범위·작은 변경 여부의 의미 판단은 같은 Codex의 지침/리뷰 책임이며, 실제 제한 권한과 종단 수리 성공의 검증을 대신하지 않는다.
...
- 정확한 커밋·검증·영향·복구안을 담은 배포 승인 요청을 기존 main 샘 Telegram으로 보냈다. 요청 영수증은 `success=true`, message `1179`, `mirrored=true`이며 main `/home/chaconne/.hermes/state/exdigm-factual-evidence-deploy-request-20260919.json`에 보존한다. 주인님이 해당 SHA의 배포를 승인한 뒤 현재 로컬 조정실 Codex가 샘의 실행 소유를 확인하기 전에 공식 배포를 먼저 시작했다. 샘은 같은 커밋의 배포 진행을 발견해 중복 실행을 중단하고 사후 검증과 보고만 수행했다. 따라서 실제 배포 실행자는 로컬 조정실이며 샘이 직접 배포한 것으로 기록하지 않는다. 별도의 Hermes 결과 보고는 message `1195`로 전달했고 영수증은 main `/home/chaconne/.hermes/state/exdigm-factual-evidence-deploy-result-20260919.json`에 보존한다. 이후 샘이 전달한 배포 요청은 승인 답변의 도착 채널과 무관하게 샘만 실행하며 다른 조정실은 답변 인계와 완료 확인만 한다. 제품 배포와 별개로 main의 제한 설정·격리 종단 검증·자동수정 timer 활성화는 아직 하지 않았다.
