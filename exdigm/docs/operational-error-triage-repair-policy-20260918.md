...
**최신 상태 — 2026-09-21:** 기존 메인서버 자동 수정기는 60초 타이머로 동작 중이다. 주인님의 후속 지시에 따라 조사 자체의 승인 요구를 제거하고 **자동 조사·단순 수정·검증·커밋 / 복잡한 수정 방향 승인 / 최종 배포 승인**을 구분하는 정정본의 개발 수정·검증·직접 리뷰를 마쳤다. 정정본은 운영 적용 승인 전이며 실제 실행기·샘에는 아직 적용하지 않았다. 68개 관련 검사 통과 및 현재 재개 정보는 10절의 「조사 자동화와 두 승인 시점 정정」을 따른다. 아래 설치·자동 실행 재개 당시 기록은 이력으로 보존한다.
