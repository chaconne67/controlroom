...
**최신 결정 — 2026-09-21:** Exdigm은 오류 사실만 기존 DB에 기록한다. 메인 서버의 주기 실행 코드가 DB를 읽어 main Codex를 호출하고, main Codex가 조사·수정 방향 결정·승인된 수정·검증·Git·커밋·승인된 운영 배포·배포 후 확인을 모두 수행한 뒤 결과를 DB에 기록한다. **샘은 DB의 처리 결과를 주인님께 보고하고, 주인님의 승인·거절·보류를 DB에 기록하는 통신만 담당한다.** 샘은 Codex를 호출하거나 조사·수정·검증·커밋·배포하지 않는다. 현재 main 자동 수정 타이머는 비활성 상태이고 샘의 정기 보고 작업도 정지 상태이며, 이 역할 정정으로 어느 쪽도 활성화하지 않는다. 현재 재개 정보와 검증 범위는 10절의 최신 항목을 따른다. 아래 설치·자동 실행 재개 당시 기록은 이력으로만 보존한다.

작성일: 2026-09-18 KST. 개정: 2026-09-21 KST. 현재 상태: **기존 제품 오류 기록은 보존한다. main 자동 수정 타이머는 비활성 상태이고 샘의 정기 보고 작업은 정지 상태다. 과거 설치·활성화와 시험 결과는 아래 이력에 남기되 현재 실행 상태나 역할의 근거로 사용하지 않는다. 샘이 수행하도록 만들었던 수정·배포 책임은 폐기한다.**

## 1. 목표와 확정된 경계

오류 한 건을 보면 **무슨 업무가 어디에서 멈췄고, main Codex가 무엇을 조사해 어떤 조치를 정했으며, 무엇을 확인해야 끝나는지** 알 수 있게 한다. Exdigm은 발생한 곳에서 사실을 공통 함수로 한 번 기록하고, 오류의 의미·원인·후속 조치는 main Codex가 한 번 판단한다. 다른 스크립트나 워커가 업무 테이블을 다시 찾아 같은 판단을 만드는 구조를 공식 경로로 남기지 않는다.

주인님이 결정한 자동 범위는 **수정·검증·리뷰·커밋까지 자동, 배포는 승인**이다. 큰 구조·업무 규칙·권한·데이터 의미 변경은 적용 전에 구체적 변경안을 승인받는다. 승인 뒤의 수정과 배포 실행도 main Codex가 담당한다. 사용자 알림과 승인 창구는 **메인 서버 Hermes 샘(`sam1065_bot`)**이다. 로컬 PC의 Hermes와 Exdigm 직원별 Hermes는 이 경로와 무관하며 건드리지 않는다.

2026-09-21 주인님이 승인 시점을 재확정했다. **원인 조사·자료 확보·기존 권한 안의 격리 재현·수정 방향 결정은 자동**이다. 단순한 결함은 검증·리뷰·커밋까지 진행한 뒤 최종 배포만 승인받는다. 복잡한 문제도 먼저 원인과 대안을 조사하고, 확인된 원인·권장 수정안·대안과 선택 이유·영향·검증·복구를 갖춘 뒤 **수정 방향 승인 → 구현·검증·커밋 → 최종 배포 승인**으로 진행한다. 조사할 계획이나 불확실성 자체를 수정 방향 승인으로 바꾸지 않는다. 이 정책 변경은 아래 재개 절의 적용 상태와 구분한다.

`unclassified`는 조사 결과가 아직 확정되지 않았다는 뜻이다. 확보 가능한 증거와 허용된 재현 경로를 같은 실행에서 조사한다. `user_action`은 주인님만 제공할 수 있는 자료·인증·새 권한·업무 선택이 실제로 필요할 때 사용하고, 조사한 내용·막힌 단계·허용된 대안의 결과·최소 사용자 행동과 재개 조건을 기록한다. 서버 담당자나 외부 조건 대기는 `external_wait`다. 두 상태는 승인·거절·보류 요청이 아니며, 샘은 현재 `approve_change`·`approve_deploy`에만 승인 결정을 요청한다. 자동 실행 자체의 중단도 실행 소유를 보존한 채 담당자가 상태를 대조할 문제이며 조사 승인으로 바꾸지 않는다.

2026-09-21 주인님이 역할을 다시 확정했다. **Exdigm 코드는 실패 사실을 기록하고, main의 연결 코드는 DB를 조회해 Codex를 호출하며, main Codex는 조사·수정·검증·리뷰·커밋·승인 요청 작성·승인된 운영 배포·배포 후 확인을 맡는다. 샘은 처리 결과 보고와 주인님의 승인·거절·보류를 DB에 기록하는 일만 맡는다.** 주인님은 큰 변경과 배포 여부를 결정한다. 샘과 main Codex는 서로 직접 통신하거나 호출하지 않고 기존 오류 DB의 같은 사건과 처리 이력으로 인계한다.

같은 날 확정한 보고 기준은 **오류 발생 자체는 알리지 않고, Codex의 1차 조사·처리 결과를 보고한다**는 것이다. 초기에는 Codex가 처리한 모든 실제 사건의 결과를 보고한다. 정상적인 종료로 확인된 경우, 외부 조건 대기, 수정 불필요, 조사·수정 중단도 결과에 포함한다. 심각도나 분류를 이유로 에이전트가 보고 대상을 임의로 줄이지 않는다. 향후 보고 제외 대상은 주인님이 별도로 지정한다. main Codex가 수행한 승인된 배포·검증 결과도 샘이 보고한다.

기획을 자세히 쓰기 위해 실행 프로그램을 늘리지 않는다. 이번 정본은 **Exdigm의 공통 기록, main 연결 코드의 DB 조회·Codex 호출, main Codex의 조사·수정·검증·배포, 샘의 보고·결정 기록**이라는 실제 목적을 중심으로 정리한다. 기존 DB·주기 실행·결정 기록 도구를 재사용하며 주체 사이의 별도 직접 통신 경로를 추가하지 않는다.
...
flowchart TD
    ...
    E --> D
    D --> F[main 주기 코드가 처리할 건을 읽어 Codex 호출]
    F -->|조사·수정 결과와 승인 요청| D
    D -->|완료된 처리 결과| G[메인 서버 샘이 주인님께 보고]
    G -->|주인님의 승인·거절·보류| D
    D -->|새 결정 발견| H[main 주기 코드가 Codex 호출]
    H --> I[main Codex가 승인 범위의 수정 또는 배포·검증]
    I -->|실행 결과와 남은 업무 확인| D
```
...
|---|---|---|
| Exdigm 업무 코드와 공통 기록 함수 | 오류/업무 중단을 확정한 즉시 | traceback 또는 기록 호출 경로, 실제 파일·함수·줄, 실행 코드 판본, 원천 업무 번호·상태와 생산자가 이미 확인한 업무 사실을 한 번 기록. 최종 분류·원인·수정 판단 없음 |
| 기존 Exdigm 알림 워커 | 기존 약 5초 반복 | DB에 못 쓴 임시 기록 재전달. 새 의미 판단·수정·승인 없음 |
| main의 최소 실행 연결 코드 | DB에 조사·수정·배포할 건이 있을 때 | 현재 사건·개정·결정을 확인해 main Codex를 호출하고, Codex 결과를 같은 DB 경로로 기록. 의미 판단·수정·배포를 직접 구현하지 않음 |
| main Codex | 연결 코드가 현재 DB 행동에 따라 호출할 때 | 자료 확인→분류·원인 조사→수정 방향 결정→승인된 수정→검증·리뷰·Git·커밋→승인된 공식 배포→배포 후 확인. 각 단계의 결과와 다음 승인 요청을 DB에 기록 |
| 메인 서버 샘 | 처리 결과는 10분마다 확인, 사용자 답변은 수신 시 처리 | 완료된 조사·처리 결과 보고, 현재 요청 설명, 주인님의 승인·거절·보류를 같은 DB에 기록하고 종료. 조사·수정·검증·커밋·배포·Codex 호출 없음 |
...
현재 운영값은 **이전 실행이 끝난 뒤 60초마다 처리 대기 확인, 동시 Codex 1개, Codex 호출 3,500초 상한, 같은 사건·같은 근거로 자동 호출 1회**다. 선택 검사는 1,800초, Django check는 300초로 제한하고 systemd 서비스 전체 상한은 120분이다. `OnUnitInactiveSec=60s`이므로 한 건이 오래 걸려도 다음 주기와 겹치지 않고 그 실행이 끝난 60초 뒤 다음 건을 확인한다. 상한에 걸리면 미완료 자료와 재개 조건을 기록하고 같은 입력을 무한 재실행하지 않는다.
...
업무 코드가 이미 확인한 구조화된 사실은 공통 기록 함수가 조사 증거로 그대로 연결한다. 예를 들어 `EXPECTED_MISSING_DATA_STATES`, 기존 `notify_resume_missing_fields`, 로그인 링크 열람 실패의 담당자·원본·재개 경로는 생산자 사실이다. 이 사실만으로 Exdigm이 C1~C4나 다음 행동을 확정하지 않으며 최초 기록은 `unclassified/investigate`로 둔다. main Codex가 원본과 현재 코드를 함께 조사해 최종 분류와 후속 조치를 정한다. 오류 문구 키워드 분류표나 규칙 스크립트를 만들지 않는다.
...
| 분류와 근거 | main Codex: 생산자가 남긴 업무 사실과 원본·코드·실행 증거를 조사해 C1~C5, 판단 시각·근거·확인 정도를 기록. 반증 뒤 수정은 이력으로 남김 |
| 원인 | Codex: 관측 현상·가설·확인 원인을 구별. 최초 이탈 단계·증거 참조. 모르면 미확인 |
| 현재 다음 행동 | 해당 처리 주체: 무엇을 누가 해야 하는지, 대상 자료/커밋, 재개 조건과 개정 번호 |
| 조사·수정·배포 결과 | main Codex: 실행 번호, 기준·결과 커밋, 실제 조사·수정·검증·리뷰·배포, 성공/실패·남은 일 |
| 변경/배포 요청 | 같은 오류의 현재 행동과 이력: 요청 번호·개정, 구체적 변경 범위·영향·증거·복구안. 별도 승인 장부를 중복 구현하지 않음 |
| 사용자 결정 | 메인 서버 샘: 누가 언제 어떤 요청/개정/커밋에 승인·거절·보류했는지와 지시 근거 |
...
|---|---|---|
| 1. 기록 | Exdigm이 발생 즉시 공통 함수 호출 | 원래 오류, traceback/기록 호출 경로, 실제 위치·코드 판본·업무 번호와 생산자 사실. 최초 분류/행동은 `unclassified/investigate` |
...
| 5. 커밋과 요청 | 같은 Codex가 검토한 변경을 커밋하고, 연결 코드가 그 clean 커밋에서 공식 `test`·`check`를 직접 실행한 뒤 결과 대조·보관·저장 후 작업 공간 반환 | 수정 내용·검사 출력/종료 상태·기준/결과 커밋·보관 참조·배포 승인 요청. 운영은 아직 고치기 전 상태이며 다음 사건 처리 가능 |
| 6. 전달·결정 | 샘이 새 행동/결과를 알리고 주인님이 결정하며, 샘이 그 승인·거절·보류를 DB에 기록하고 종료 | 정확한 요청·개정·변경안/커밋에 대한 결정 기록. 메시지 전달·결정 기록·실행은 서로 다른 사실 |
| 7. 승인된 실행 | main 연결 코드가 새 결정을 읽어 main Codex를 호출하고, Codex가 승인된 수정 또는 배포를 수행 | 수정 승인과 배포 승인 각각의 실행 결과. 실제 운영 판본·서비스 확인은 배포 뒤 DB에 기록 |
...
Codex 종료 코드 0이나 JSON 형식 통과를 실제 수리 성공으로 취급하지 않는다. 연결 코드는 보고된 SHA가 실제 clean debug HEAD인지 먼저 대조한다. Codex는 원래 실패·같은 원인의 변형·관련 기존 성공을 확인할 구체적인 pytest 파일 또는 node ID를 고르고, 검사가 없으면 회귀 검사를 추가한다. 실행기는 그 대상이 저장소 안의 추적된 Python 검사인지 다시 확인한 뒤 공식 `scripts/debug_workspace.sh test <대상>`과 `check`를 직접 실행한다. 종료 상태 0과 출력·대상 목록·커밋을 실행별 영수증에 저장한 경우에만 배포 승인 요청을 기록한다. 현재 전체 테스트 기준선은 제품 변경과 무관한 기존 실패가 있어 작은 수정의 성공 판정에 사용하지 않으며, 기존 실패를 성공으로 숨기거나 삭제하지 않는다.
...
보고는 기존 `processing_history`의 `succeeded`/`failed` 결과를 사용한다. 여기서 succeeded는 해당 처리 단계의 결과이며 원래 업무의 복구 완료를 자동으로 뜻하지 않는다. 확인한 문제, 원인과 근거 또는 남은 불확실성, 실제 처리 또는 제안, 검증 결과, 주인님의 판단·행동 필요 여부를 설명한다. 원인 미확인·수정 중단도 무엇을 확인했고 왜 멈췄으며 다음에 무엇이 필요한지 보고한다. 정기 보고에서 샘은 Codex의 결론을 추측으로 바꾸지 않고 새 조사를 시작하지 않는다.
...
같은 요청·결정을 다시 전송하면 고정된 기록 번호와 최초 저장 인수를 재사용한다. 응답 소실을 두 번째 승인으로 처리하지 않는다. 결정 기록 성공과 실제 실행 성공은 구분한다. 다른 처리로 개정이 달라졌으면 실행 허가를 반환하지 않는다. 샘은 결정을 기록한 뒤 종료한다. main 연결 코드가 새 결정을 읽을 때 현재 개정과 실행 상태를 다시 확인하고 main Codex만 승인된 작업을 진행한다.

- 변경 승인은 특정 제안의 구현 권한이다. main 연결 코드가 현재 결정과 개정을 대조해 main Codex를 호출한다. Codex는 승인된 제안과 조사 자료를 이어받아 공용 조정실 지침·기능 스킬·문제해결·SSP·최소 구현·공식 검증·리뷰 절차로 수정·커밋한다. 기존 `workspace_lock`·`run_official_verification`·`park_repair`를 재사용하고, 검사 영수증·수정본 보관·결과 저장·작업 공간 반환을 확인한 뒤 새 `approve_deploy` 요청을 DB에 남긴다. 구현 승인은 배포 승인이 아니다.
- 배포 승인은 특정 검증된 커밋의 운영 반영 권한이다. main 연결 코드가 현재 결정과 커밋을 대조해 main Codex를 호출한다. Codex는 `exdigm-deploy`와 8절의 debug 잠금·예약을 사용하고 DB의 요청 개정·`base_commit`·`commit`·`repair_ref`를 실제 Git과 대조한 뒤 공식 `scripts/deploy/deploy.sh prod`로 배포한다. 다른 미승인 변경을 섞지 않는다.
...
- 배포 중 코드 수정이 필요하면 main Codex는 실패 근거와 검증 결과를 같은 오류 이력에 남기고 현재 수정 승인 범위부터 대조한다. 승인 안의 수정은 다시 검증·커밋하고, 범위를 넘으면 구체적 수정안을 DB에 남겨 먼저 승인받는다. 새 커밋은 새 배포 요청으로 남기며 이전 커밋의 배포 승인으로 적용하지 않는다.
...
운영 DB의 분류·다음 행동·처리 이력 필드와 migration `0070`은 배포됐다. 과거 자동 수정 활성화의 종단 검증에는 일반 사건과 혼동되지 않는 고정 시험 표식의 합성 사건 한 건만 사용했고, 샘 조회가 같은 표식을 제외하는지 확인한 뒤 `expected_stop/none`으로 닫았다. 실제 사건의 승인 저장과 main Codex의 배포 검증은 이 시험 결과와 구분한다.
...
현재 설치된 main Codex용 `exdigm-repair` 제한 계정과 별도 SSH 키는 debug 수정·검증과 자기 실행 자료 기록만 할 수 있고, sudo·운영 checkout 쓰기·push·배포·임의 운영 DB 쓰기·사용자 승인 기록은 할 수 없다. 따라서 승인된 운영 배포를 main Codex가 수행하는 공식 제한 권한 연결은 아직 구현·검증되지 않은 공백이다. 그 연결이 준비되기 전에는 배포를 실행하지 않으며 샘이 대신하지 않는다. 오류 DB SSH의 제한과 GBrain 읽기 전용 경계는 유지한다.
...
`approve_deploy`는 실제 debug HEAD=전체 커밋 SHA, 시작 HEAD와 다름, clean 상태를 확인해야 한다. Codex가 출력한 검사 문장이나 CLI 명령 문자열은 실행 성공 증거로 사용하지 않는다. 같은 단건 실행기가 Codex가 고른 1~20개의 구체적인 추적 검사 대상을 검증해 공식 `scripts/debug_workspace.sh test <대상>`과 `check`를 직접 실행하고, 종료 상태 0·명령·대상 목록·출력 파일·커밋을 원자적인 `official-verification.json`에 기록한다. 중단 후 같은 커밋·같은 대상의 완료된 검사만 영수증과 출력 파일을 대조해 재사용하고, 실패·대상 변경·출력 유실은 성공으로 기록하지 않는다. 검사 범위의 의미 판단은 같은 Codex와 코드 리뷰의 책임이고, 실제 실행과 영수증 판정은 스크립트 책임이다.

## 9. 구현 순서와 최소 구현 기준

주인님의 v2 구현·배포·활성화 승인으로 아래 1~4단계를 모두 운영에 연결했다. 실제 제품 수정의 배포 승인과 원래 업무 결과 확인은 사건별로 계속 분리한다.
...
| 단계 성공·기존 행 갱신 | 커밋/검사 성공 뒤에도 배포/보완 행동 조회 가능, 후속 결과를 샘이 읽음 |
| PC 종료·전달 실패 | 사건과 요청 보존, 복귀 후 발견. 준비/조회 실패를 전달 성공으로 오기록하지 않음 |
| 권한·승인 | Codex가 사용자 승인을 만들거나 승인 전 운영 쓰기를 하지 못함. 샘이 기록한 정확한 요청 개정·커밋과 승인 범위만 main 연결 코드가 실행에 사용 |
...
### 샘 통신 전용·main Codex 단일 실행 — 2026-09-21 최신

- 정본 역할: Exdigm은 오류 사실을 기존 DB에 기록한다. main 연결 코드는 DB를 주기적으로 읽어 현재 행동이 있는 사건에 main Codex를 호출한다. main Codex가 조사·수정 방향 결정·승인된 수정·검증·Git·커밋·승인된 운영 배포·배포 후 확인을 수행하고 모두 같은 DB 이력에 기록한다.
- 샘은 완료된 DB 처리 결과를 주인님께 보고하고, 주인님의 승인·거절·보류를 기존 `decision.py`로 같은 요청 개정에 기록한 뒤 종료한다. 샘은 작업 잠금·SSH 수정·검사·리뷰·Git·커밋·서비스 조작·배포·Codex 호출을 하지 않는다.
- 최소 구현 게이트는 기존 경로 재사용에서 멈춘다. 제품 코드, DB 구조, 결정 기록 코드, 모니터·체크포인트 코드, 새 서비스나 새 인계 프로그램을 만들지 않는다. 역할이 잘못 적힌 스킬·정책·보고 프롬프트만 바로잡는다.
- 현재 자동 실행 상태는 그대로 정지다. main 자동 수정 타이머는 비활성 상태이고 샘의 정기 보고 작업도 정지 상태다. 이 문서·스킬 정정은 어느 작업도 활성화하지 않는다.
- 현재 main의 제한 계정에는 운영 배포 권한이 없다. 승인된 배포를 main Codex가 수행할 공식 제한 권한 연결은 별도 설계·검증이 필요한 미완료 항목이며, 준비되기 전까지 샘 또는 다른 주체가 우회 실행하지 않는다.
- 이 항목이 현재 역할의 정본이다. 아래의 샘 직접 수정·배포 문구와 과거 활성화 기록은 당시 상태를 보존한 역사 자료이며 현재 지침으로 사용하지 않는다.

### 과거 결정(폐기): 승인된 수정은 샘이 직접 수행 — 2026-09-21

아래 기록은 잘못된 역할 결정이 어떻게 생겼는지 확인하기 위한 이력일 뿐이다. 샘의 현재 행동이나 권한의 근거로 사용하지 않는다.

- 주인님이 샘의 조정실 자원을 활용해 승인된 수정을 직접 수행하도록 방향을 정했다. 초기 무인 조사·단순 수정은 기존 Codex 실행기에 남기고, 수정 방향 승인 뒤 별도 Codex CLI로 다시 넘기는 연결을 만들지 않는다.
- 기준선은 로컬 Controlroom `bca7e40`, main `7848678`이다. 다른 프로젝트 문서의 기존 변경은 보존한다. 범위는 `exdigm-error-control/SKILL.md`, 샘 보고 프롬프트, 이 정책과 문서 목차다. 프로그램·모델·인증·도구 권한·예약·서비스·제품 코드를 바꾸지 않는다.
- 현재 사실: 샘의 `controlroom-work`는 공용/프로젝트 지침·GBrain·기능 스킬·Exdigm SSH 및 공식 검사 경로를 안내한다. 샘의 실제 스킬 목록에 두 스킬이 있고 현재 Telegram 세션에 terminal/read_file/patch/skill_view 도구가 있다. 같은 main 사용자로 Exdigm SSH·debug Git 조회·쓰기 가능한 작업 폴더·공식 검사 진입점과 조정실 지침의 읽기를 확인했다. 이것은 접근 준비 확인이며 샘의 실제 수정 수행 결과는 아니다.
- 모델은 설정과 실제 실행 기록으로 구분했다. 현재 샘 설정·열린 Telegram 세션은 `gpt-5.6-sol`/`openai-codex`, 자동 수정기의 최근 실제 두 실행은 `gpt-6-astra`였다. 같은 모델이라는 전제나 동일한 작업 품질을 검증 사실로 기록하지 않는다. 이번 결정은 같은 조정실 자원과 검증 절차를 사용하는 실행 책임의 단순화이며 모델 변경을 포함하지 않는다.
- 최소 구현 게이트: 2단계에서 멈춤. 기존 승인 기록·공용 debug 잠금·공식 검사 영수증·커밋 보관·오류 결과 명령을 재사용한다. 승인 도구의 `execution_allowed` 반환은 작업 시작 증거가 아니며 샘이 승인 개정 대조·고정 시작 기록·실제 실행·새 배포 요청까지 맡는다. 기존 실행 자료가 있으면 현재 상태와 대조해 이어받고 중복 writer를 시작하지 않는다.
- 검증: 수정 전후 `quick_validate.py`를 UTF-8 모드로 실행해 통과했다. `skill-review`로 승인·거절·보류·개정 변경·다른 writer·커밋 후 중단·별도 배포 승인 사례의 지침과 기존 도구를 대조했고, 마지막 검토에 남은 지침 결함은 없다. 기존 승인 도구 10개 검사와 diff 검사도 통과했다. 실제 주인님 승인→샘 수정→검사→배포 요청의 종단 실행은 아직 수행하지 않았다. 68개 기존 검사를 이 새 종단 경로의 검증으로 확대하지 않는다.
- 적용 상태: 정본의 역할·절차 수정과 검토를 마쳤으며 main 샘 설치본은 이전 내용이다. 운영 적용 승인은 아직 받지 않았고 서비스 재시작·Telegram 발송·실제 오류 처리 상태 변경은 하지 않았다. 정본·main·샘 설치본을 맞춘 뒤 현재 세션의 지침 로딩과 실제 실행 결과를 별도로 확인한다.

### 조사 자동화와 두 승인 시점 정정 — 2026-09-21

- 사용자 목적: 조사를 맡긴 사람이 다시 조사 승인을 해야 하는 부담을 없앤다. 자동 조사와 단순 수정·검증·커밋, 복잡한 수정 방향 승인, 최종 배포 승인을 구분한다.
- 확인 원인: `repair_once.py:prompt_for`가 정보/권한 부족을 일괄 `unclassified/user_action`으로 종료하도록 지시했다. 샘 보고 프롬프트도 승인·거절·보류의 대상을 승인 행동 두 종류로 제한하지 않았다. 실행 결과의 행동 목록은 종료 결과용이므로 조사 계속용 상태나 작업자를 늘리는 대신 같은 실행 안에서 허용된 조사를 계속하게 한다.
- 실제 사례: 오류 `60ead5b9-11ab-0359-769d-cff3f675ba66`의 실행 `b1d28b13-3a77-4581-989f-a7a7b35107b2`는 `unclassified/user_action`으로 종료됐다. 당시 생성 응답 부재와 pytest 임시 폴더 UID 불일치를 기록했다. 후자는 아래 재개 기록에서 이미 복구·15개 검사 통과했으므로 현재의 조사 승인 사유로 반복하지 않는다. 이력서 원인·저장 복구는 아직 확인되지 않았다.
- 기준선: Controlroom `7848678d18e4118a97a9f4c1967876da04e81285`. 로컬의 다른 프로젝트 문서 수정은 보존한다. main Controlroom은 clean, Exdigm 운영/debug는 clean `649aa4e692556ffe2c84340897fc29350d1d33eb`이고 원격 작업 예약이 없었다. 변경 전 실행/권한 검사 38개와 실제 Hermes Python의 승인 검사 10개가 통과했다.
- 범위/공식 경로: 기존 단건 실행기 `prompt_for → Codex → validate_result → 기존 결과 기록`, 기존 샘 보고 프롬프트, 관련 검사와 이 문서. 제품 코드·DB 구조·승인 인증·작업 소유·배포·운영 데이터·다른 사용자 작업을 보존한다. 자동 실행 연결 실패는 예약을 유지하고 `external_wait`로 기록하며 사용자 승인으로 전환하지 않는다.
- 최소 구현 게이트: 2단계에서 멈춤. 기존 `root_cause`, `verification`, `stop_reason`, `owner`, `required_action`, `resume_condition`과 결과 저장·잠금·승인 경로를 재사용한다. 기존 코드의 `ACTIONS`, `validate_result`, 제품의 `record_processing_result` 및 `claim_automatic_repair`, 샘 `prompt-fragment.txt`를 직접 대조했다. 새 필드·의존성·재시도 작업자를 만들지 않는다.
- 역할/출력 계약: Codex가 원인·난이도·실제 필요한 외부 행동을 판단한다. 실행기는 필수 증거의 존재를 검사한다. 변경 방향 승인에는 원인과 검토 가능한 제안, 사용자 행동/외부 대기에는 조사 결과와 구체적 중단 근거가 필요하다. 근거가 없으면 기존 1회 출력 정정 뒤 명시적 중단 경로를 따른다.
- 검증/리뷰 계약: 위 base 대비 해당 파일과 직접 소비자인 결과 기록·샘·승인 도구만 검토한다. 기존 실행/권한/승인 검사를 보존하고 조사 없는 변경 승인 거절, 증거 없는 사용자 행동/외부 대기 거절, 검토 가능한 변경안 인계, 실행 중단의 담당자 대기와 예약 유지, 단순 수정의 배포 승인 인계를 확인한다. 의미 판단은 코드로 분류하지 않으며 지침의 실제 준수 여부는 별도 실행 검증으로 구분한다.
- 수정 후 검증: 격리된 main 임시 경로에서 실행/권한 검사 41개 통과. 승인 도구 10개와 샘 결과 조회/전달 검사 17개도 통과했고 마지막 검사는 공식 운영 읽기 전용 DB의 SELECT 안에 만든 가상 자료로 실제 SQL/JSON 계약을 확인했다. 합계 68개, Python 구문 검사와 diff 공백 검사 통과. 시스템 Python에 없는 테스트 의존성은 기존 Hermes Python·기존 pytest 환경을 사용해 해결했으며 런타임 의존성을 설치하지 않았다.
- 직접 코드 리뷰: 기존 공식 검증·커밋 보관·실행 소유·중단 재전송·제한 결과 저장·샘 승인 인증을 대조했고 승인된 finding과 열린 계약 질문이 없다. 자동 실행 결과의 의미 판단은 LLM에 남겨 두었다. 이번 검사는 연결/출력 계약과 보고 지침 검토이며 새 지침을 실제 운영 Codex/Sam이 준수하는 종단 결과까지 확인한 것은 아니다.
- 적용 상태: 개발 수정·검증·리뷰 완료. 실행 중인 main 자동 수정기와 샘 예약에는 아직 적용하지 않았다. 주인님의 최종 배포 승인 뒤 기존 공식 설치 경로에 같은 판본을 적용한다. 위 사건의 조사는 이미 허용된 범위이며 별도 조사 승인이 필요하지 않다. 자동 실행기로 인계할 때는 최신 DB 개정·이력·종료 writer·기존 예약을 다시 대조한 뒤 기존 결과 명령을 사용한다. 과거 승인 요청을 새 정책의 배포 승인으로 간주하지 않는다. 고객 업무의 운영 재처리와 저장 복구는 이 정책 정정의 성공으로 대신 기록하지 않는다.

### 보안 보완 후 자동 실행 재개 — 2026-09-21

- 사용자가 기존 자동 수정 기능을 다시 동작하도록 명시적으로 지시했다. 새 실행기·승인 화면·봇을 추가하지 않고 설치된 서비스와 타이머를 재사용한다. 기존 제한 계정과 원격 개발 공간, 공식 검사·결과 기록·수정본 보관·샘 승인 절차를 유지한다.
- 실행 전 남은 main 실행 상태·원격 예약·미배포 수정본이 없고 운영/debug가 같은 clean 판본임을 확인했다. 메인서버의 실행 계정을 일반 업무 프로세스와 분리하고 원래 인증·파일 내용·SSH·그룹 권한을 보존했다. 기존 DB 프로세스를 정지하거나 DB 파일 소유자를 바꾸지 않았다.
- 같은 제한 계정으로 사전 검사와 Codex 실행, 서비스 설정 검사를 통과했다. 설치된 서비스가 실제 대기 사건 한 건을 확보하여 Codex 조사 결과를 같은 사건의 다음 개정에 저장하고 정상 종료했다. 원인 근거가 부족한 사건이어서 추가 확인 요청으로 기록했으며 불필요한 코드 수정·배포·고객 업무 재처리는 하지 않았다.
- 첫 실제 조사에서 기존 pytest 임시 폴더의 소유권 불일치로 회귀 검사 시작이 거절된 사실을 확인했다. 해당 전용 임시 폴더만 내용·권한을 보존해 현행 실행 계정으로 이관하고, 같은 main 제한 SSH와 공식 검사 명령으로 15개 검사를 모두 통과했다. 해당 사건의 원천 자료 부족과 자동 수정 검증 환경의 복구를 구분한다.
- 기존 타이머는 active/enabled로 복구했다. 이전 실행이 끝난 뒤 60초에 다음 건을 확인하고 한 번에 한 건만 처리한다. 처리 결과 기록과 원래 고객 업무의 복구 완료는 계속 구분한다. 후속 실제 코드 수정은 해당 사건의 검증 결과와 배포 요청으로 따로 확인한다.
- 자동 수정 권한에 대한 보완과 샘의 추가 배포 승인 구조 개선을 분리한다. 새 승인 채널을 선택하지 않았다는 이유로 자동 수정을 다시 정지하지 않는다. 추가 위험을 발견하면 영향을 확인해 해당 범위를 보완하고 기존 자동 기능을 보존한다.
- 상세 보안 검증·복구 위치는 [내부 보안 실행 기록 안내](security-audit-20260920.md)를 따른다. 상세 문서를 공개 저장소에 포함하지 않는다.
...
실제 설치·실행 확인:
- Controlroom `9cbf3d9c4cf33950ca9295718d9ba22223b5d8e5`를 로컬/main에 동기화했다. main에서 별도로 작성된 위 가동 준비 검증 기록도 원문 그대로 보존했다. 조회 스크립트와 전달 확인 스크립트의 설치본/정본 SHA-256을 대조하고, 기존 예약의 prompt만 갱신했다.
- 샘 job `130323c787e0`의 10분 간격·skill·script·origin·deliver·모델·provider·대화 연결·workdir는 설치 전 백업과 같고, 기존 상태 파일도 바이트 단위로 같다. 작업은 다시 enabled/scheduled이며 gateway가 active다.
- main의 실제 `collect_context`는 보고할 사건 0건을 반환했다. 기존 native `hermes cron run` 실행 `5d75a656520f47a594164d6bdf3103d8`은 `completed/suppressed`다. 보고할 결과가 없을 때 발송하지 않는 실제 경로를 확인했으며, 실제 처리 결과 메시지의 생성/전달 성공을 주장하지 않는다. 시험 오류나 사용자 알림은 생성하지 않았다.
- 공용 GBrain `project/exdigm-operational-error-alerts`에 최신 결과 보고 기준을 추가하고 이전 본문 전체가 보존됐는지 다시 읽어 확인했다. 제품 코드·운영 DB·수리용 신원·권한·timer는 이번 작업에서 변경하지 않았다. 실제 무인 수리 활성화와 그 처리 결과 전달의 종단 검증은 위 가동 준비 절의 남은 작업이다.
...
- main 실행기는 Codex가 제시한 clean 커밋과 선택 검사 대상을 검증한 뒤 공식 `scripts/debug_workspace.sh test <대상>`와 `check`를 SSH로 직접 실행한다. 현재 제품 전체 테스트 기준선은 `2912 passed, 9 skipped, 224 failed, 42 errors`로 이미 red이므로 전체 suite를 작은 수정의 승인 차단기로 사용하지 않는다. 운영 오류 관련 실제 선택 검사 두 파일은 제한 계정으로 `79 passed, 5 warnings`, Django check는 문제 없음이었다. 기존 실패를 고치지 않고 성공으로 간주하지도 않는다.
- Controlroom 결과는 `31ca5144c11dfc6798c17c68fbc456e3d671a88c`이며 로컬·GitHub·main `/home/chaconne/controlroom`이 같다. 실행기·제한 SSH 경계·검사 영수증·중단/재전송·Git 보관 계약 35개 단위검사와 Python compile, systemd unit 검증을 통과했다. code-review-loop에서 서비스 상한보다 내부 최장 경로가 길던 문제와 검사 대상 목록의 DB 문자열 계약 불일치를 수정했고, 재검토에 남은 승인 finding이나 열린 계약 질문은 없다.
- main의 전용 계정·키·known_hosts·`/etc/exdigm-repair/config.json`, Exdigm의 제한 DB 기록 환경, 양쪽 강제 SSH 진입점, systemd service/timer를 설치했다. 실제 Codex가 GBrain 정본과 운영/debug HEAD `e0606504859b10298456387ef17780b5ce6d4c68`을 읽는 데 성공했다. Codex CLI 자체 샌드박스는 상위 `.git`과 SSH를 막아, OS 전용 계정·systemd·강제 SSH를 외부 보안 경계로 두고 `danger-full-access`를 사용한다.
- 합성 사건 `03fb90c7-3ce0-5e24-8fc1-40d274f10648`, 실행 `b65429f1-fae8-4a2c-ba42-1d5987ab65cc`에서 main Codex가 `expected_stop`으로 판정하고 같은 DB에 결과를 저장했다. 서비스는 exit 0, 1분 56초, 최대 메모리 약 214MB였고 `active.json`·원격 예약·debug 변경이 남지 않았다. 사후 대조 뒤 시험은 개정 4의 `none`으로 닫았으며 샘 SQL의 고정 시험 표식 제외도 확인했다.
- `exdigm-repair.timer`는 2026-09-19 23:56:56 KST부터 enabled/active다. 첫 자동 조회는 `no_work` exit 0이었고 다음 확인 시각이 60초 뒤로 잡혔다. 실행 중에는 다음 주기가 겹치지 않으며 승인 대기 커밋은 보관 ref로 옮기고 작업공간을 비워 다음 사건을 처리한다.
- 정확한 커밋·검증·영향·복구안을 담은 배포 승인 요청을 기존 main 샘 Telegram으로 보낸 이전 영수증은 message `1179`, 결과 보고는 message `1195`로 보존돼 있다. 그 초기 배포는 로컬 조정실이 먼저 실행해 샘이 중복 실행을 중단한 사례다. 이후부터 샘이 전달한 배포 요청은 승인 답변의 도착 채널과 무관하게 샘만 실행하며 다른 조정실은 답변 인계와 완료 확인만 한다. 현재 main 자동 수정은 배포 명령·운영 쓰기 권한을 갖지 않는다.
