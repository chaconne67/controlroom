...
**최신 상태 — 2026-09-21:** 주인님의 자동 수정 복구 지시에 따라 메인서버의 기존 실행기를 재개했다. 실제 대기 사건 한 건의 main Codex 조사·결과 저장·작업 공간 반환을 확인했고, 기존 60초 타이머와 서버 시작 시 자동 실행을 활성화했다. 수정·검증·리뷰·커밋까지 자동, 배포는 기존 샘 승인 경로라는 범위를 유지한다. 별도 승인 웹 화면이나 봇의 도입은 자동 수정 재개의 조건이 아니다. 아래 9월19일 설치 당시 코드 판본·시험 기록은 이력으로 보존한다.
