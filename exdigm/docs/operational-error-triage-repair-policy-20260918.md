...
**최신 상태 — 2026-09-21:** 기존 메인서버 자동 수정기는 60초 타이머로 동작 중이다. 조사 자체의 승인 요구를 제거하는 정정본은 개발 커밋 `bca7e40`에 저장했고 68개 관련 검사가 통과했다. 후속 사용자 결정에 따라 **초기 자동 조사·단순 수정은 main Codex / 승인된 복잡한 수정은 샘이 직접 구현 / 최종 배포는 별도 승인 뒤 샘**으로 실행 책임을 정리한다. 정정본과 샘의 직접 수정 절차는 실제 실행기·샘 설치본에 아직 적용하지 않았다. 현재 재개 정보와 검증 범위는 10절의 최신 두 항목을 따른다. 아래 설치·자동 실행 재개 당시 기록은 이력으로 보존한다.

작성일: 2026-09-18 KST. 개정: 2026-09-19 KST. 상태: **운영 활성화 완료. 사실 기록 보완 제품 커밋 `e0606504859b10298456387ef17780b5ce6d4c68`이 운영·debug·origin/main에 일치한다. 자동 수정 실행기는 Controlroom `31ca5144c11dfc6798c17c68fbc456e3d671a88c`로 로컬·GitHub·main을 맞췄고, 전용 제한 계정·강제 SSH 명령·systemd service/timer를 설치했다. 합성 시험 사건 `03fb90c7-3ce0-5e24-8fc1-40d274f10648`에서 기록→소유→main Codex 조사→DB 결과→예약 해제→샘 보고 제외를 확인한 뒤 2026-09-19 23:56:56 KST 타이머를 활성화했다. 60초마다 이전 실행 종료 뒤 다음 건을 확인하며 동시 실행은 한 건이다. 수정·검증·리뷰·커밋까지만 자동이고 배포는 계속 주인님의 승인과 샘의 실행을 거친다.**
...
2026-09-21 주인님이 승인 후 수정의 실행자를 샘으로 정했다. **Exdigm 코드는 실패 발견·즉시 기록, main Codex는 초기 자동 분석·단순 수정·검증·리뷰·커밋·승인 요청 작성, 샘은 결과 보고·승인 접수·승인된 복잡한 수정의 직접 구현·검증·리뷰·커밋·새 배포 요청·승인된 배포·결과 통보**를 맡는다. 주인님은 큰 변경과 배포 여부를 결정한다. 양쪽 에이전트는 같은 조정실 지침·스킬·SSH 개발 경로·작업 소유·검증 계약을 사용하며 기존 오류 DB의 같은 사건과 처리 이력으로 인계한다. 9월19일의 모든 수정은 Codex에 전달한다는 역할 구분은 이 결정으로 대체한다.
...
기획을 자세히 쓰기 위해 실행 프로그램을 늘리지 않는다. 이번 정본은 **Exdigm의 공통 기록, main Codex의 초기 자동 조사·수정, 기존 샘의 소통·승인된 수정·배포**라는 실제 목적을 중심으로 정리한다. 샘은 기존 조정실 지침과 도구를 직접 사용하며 승인 후 Codex CLI를 다시 시작하는 별도 인계 프로그램을 추가하지 않는다.
...
flowchart TD
    ...
    G -->|주인님이 수정 방향 승인| H[샘이 조정실에서 수정·검증·리뷰·커밋]
    H -->|수정 결과와 새 배포 요청| D
    G -->|주인님이 정확한 커밋의 배포 승인| I[샘이 공식 배포·결과 검증]
    I -->|배포 결과와 남은 업무 확인| D
```
...
| 메인 서버 샘 | 처리 결과는 10분마다 확인, 사용자 답변은 수신 시 처리, 수정·배포는 각각 해당 승인 뒤 | 조사·처리 결과 전건 보고와 승인 접수, 승인된 복잡한 수정의 직접 구현·검증·리뷰·커밋 및 새 배포 요청, 승인된 커밋의 공식 배포·결과 검증·통보. 정기 조회 자체는 읽기·보고만 수행 |
...
| 조사·수정 결과 | 해당 실행 소유자(main 자동 실행은 Codex, 승인 후 직접 수정은 샘): 실행 번호, 기준·결과 커밋, 실제 수정·검증·리뷰, 성공/실패·남은 일 |
...
| 7. 승인된 실행 | 샘이 수정 방향 승인 후 직접 구현·검증·커밋하고 새 배포 요청. 별도 배포 승인 뒤 샘이 공식 배포 | 수정 승인과 배포 승인 각각의 실행 결과. 실제 운영 판본·서비스 확인은 배포 뒤 기록 |
...
보고는 기존 `processing_history`의 `succeeded`/`failed` 결과를 사용한다. 여기서 succeeded는 해당 처리 단계의 결과이며 원래 업무의 복구 완료를 자동으로 뜻하지 않는다. 확인한 문제, 원인과 근거 또는 남은 불확실성, 실제 처리 또는 제안, 검증 결과, 주인님의 판단·행동 필요 여부를 설명한다. 원인 미확인·수정 중단도 무엇을 확인했고 왜 멈췄으며 다음에 무엇이 필요한지 보고한다. 정기 보고에서 샘은 Codex의 결론을 추측으로 바꾸지 않는다. 승인 후 직접 수정 중 새 증거가 확인되면 기존 기록을 보존하고 판단 변경과 영향을 같은 사건의 후속 이력에 기록한다.
...
- 변경 승인은 특정 제안의 구현 권한이다. 샘이 승인된 제안과 조사 자료를 이어받아 공용 조정실 지침·기능 스킬·문제해결·SSP·최소 구현·공식 검증·리뷰 절차로 직접 수정·커밋한다. 기존 `workspace_lock`·`run_official_verification`·`park_repair`를 재사용하며, 같은 잠금과 실행 소유를 확보한 뒤 시작하고 실제 검사 영수증·수정본 보관·결과 저장·작업 공간 반환을 확인한 뒤 새 `approve_deploy`로 인계한다. 구현 승인은 배포 승인이 아니다.
...
- 배포 중 코드 수정이 필요하면 샘은 실패 근거와 검증 결과를 같은 오류 이력에 남기고 현재 수정 승인 범위부터 대조한다. 승인 안의 수정은 샘이 직접 검증·커밋하고, 범위를 넘으면 구체적 수정안을 먼저 승인받는다. 새 커밋은 새 배포 요청으로 남기며 이전 커밋의 배포 승인으로 적용하지 않는다.
...
운영 DB의 분류·다음 행동·처리 이력 필드와 migration `0070`은 배포됐다. 자동 수정 활성화의 종단 검증에는 일반 사건과 혼동되지 않는 고정 시험 표식의 합성 사건 한 건만 사용했고, 샘 조회가 같은 표식을 제외하는지 확인한 뒤 `expected_stop/none`으로 닫았다. 실제 사건의 승인 저장과 샘의 배포 검증은 이 시험 결과와 구분한다.
...
main Codex에는 전용 `exdigm-repair` 계정과 별도 SSH 키만 연결했다. 이 계정은 debug 수정·검증과 자기 실행 자료 기록은 할 수 있지만 sudo, 운영 checkout 쓰기, main 브랜치·Git 설정·hook 변경, push, 배포, 임의 운영 DB 쓰기와 사용자 승인 기록은 할 수 없다. 오류 DB SSH는 claim/read/소유한 결과 저장 세 명령만 강제하고 GBrain은 default source 읽기만 허용한다. 실제 계정으로 이 허용·거부 경계와 두 checkout의 clean 동일 HEAD를 확인했다.
...
### 승인된 수정은 샘이 직접 수행 — 2026-09-21 후속
