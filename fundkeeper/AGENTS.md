...
- 작업 위치는 현재 조정실 루트의 실제 `fundkeeper` 폴더입니다. PC·노트북은 `~/projects/fundkeeper`, main 서버 조정실은 `~/controlroom-workspaces/fundkeeper`입니다. `~`는 Windows의 `USERPROFILE`, macOS·Linux의 `HOME`입니다.
...
- 기획·리서치·작업 계획은 `docs/README.md`에서 찾습니다. 정본은 현재 프로젝트의 실제 `docs` 폴더이며 GitHub controlroom 저장소의 `fundkeeper/docs`와 같은 위치입니다.
...
| 옛 공개 입구 | `49.247.38.186` 정지; 옛 `/home/docker/deploy.sh` 실행 금지 |
...
- 조정실 진입 폴더는 `C:\Users\chaconne\projects\fundkeeper`, main 코드 저장소는 `/home/chaconne/projects/fundkeeper`입니다. main의 서버 조정실 진입 폴더는 `/home/chaconne/controlroom-workspaces/fundkeeper`입니다. 조정실은 지침·스킬·기획의 진입점이고 서버 폴더는 실제 `.git`과 코드를 가진 독립 저장소입니다.
