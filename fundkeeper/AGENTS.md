...
- 작업 위치는 현재 조정실 루트의 실제 `fundkeeper` 폴더입니다. 모든 OS의 PC·노트북·main 서버 조정실은 `~/controlroom/fundkeeper`입니다. `~`는 Windows의 `USERPROFILE`, macOS·Linux의 `HOME`입니다.
...
1. 프로젝트 카드가 지정한 GBrain 맥락을 확인합니다.
...
- 조정실 진입 폴더는 `C:\Users\chaconne\controlroom\fundkeeper`, main 코드 저장소는 `/home/chaconne/projects/fundkeeper`입니다. main의 서버 조정실 진입 폴더는 `/home/chaconne/controlroom/fundkeeper`입니다. 조정실은 지침·스킬·기획의 진입점이고 서버 폴더는 실제 `.git`과 코드를 가진 독립 저장소입니다.
