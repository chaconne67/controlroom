...
- 작업 위치는 현재 조정실 루트의 실제 `ziin` 폴더입니다. PC·노트북은 `~/projects/ziin`, main 서버 조정실은 `~/controlroom-workspaces/ziin`입니다. `~`는 Windows의 `USERPROFILE`, macOS·Linux의 `HOME`입니다.
...
| 옛 공개 입구 | `49.247.45.243` 정지; 옛 writer는 정지 |
...
- 조정실 진입 폴더는 `C:\Users\chaconne\projects\ziin`, main 코드 저장소는 `/home/chaconne/projects/ziin`입니다. main의 서버 조정실 진입 폴더는 `/home/chaconne/controlroom-workspaces/ziin`입니다. 조정실은 지침·스킬·기획의 진입점이고 서버 폴더는 실제 `.git`과 코드를 가진 독립 저장소입니다.
