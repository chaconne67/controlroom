...
| 옛 공개 입구 | `49.247.207.147` 정지; 옛 `Rndnote` Swarm writer는 정지 |
...
- 조정실 진입 폴더는 `C:\Users\chaconne\controlroom\rndlog`, main 코드 저장소는 `/home/chaconne/projects/rndlog`입니다. main의 서버 조정실 진입 폴더는 `/home/chaconne/controlroom/rndlog`입니다. 조정실은 지침·스킬·기획의 진입점이고 서버 폴더는 실제 `.git`과 코드를 가진 독립 저장소입니다.
