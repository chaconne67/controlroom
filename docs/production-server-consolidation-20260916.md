...
## 17. 2026-09-18 00:14 KST 사용자 DNS 변경 후 실제 확인

**운영 정본 인계는 완료 상태이고 공개 DNS는 14개 이름 중 12개가 새 main `49.247.192.127`로 연결됩니다.** 주인님이 가비아에서 레코드를 수정했다고 알려 주신 뒤 실제 DNS 원본 응답과 HTTPS 경로를 읽기 전용으로 확인했습니다. `synco.kr` 및 `www.synco.kr` 두 이름은 아직 가비아 원본에서 기존 `49.247.205.170`을 반환하므로 최종 DNS 연결 전체 완료로 판정하지 않습니다.

- 가비아 권한 서버 `ns.gabia.co.kr`, `ns1.gabia.co.kr`, `ns.gabia.net` 각각에 재귀 조회 없이 직접 질의했습니다. 총 42개 A 응답이 모두 권한 응답(`aa`)이며 서버 3곳이 같은 값을 반환했습니다. Coconut, RNDLOG, rndnote, aishift, ZiiN, rogeon은 기본 이름과 www 모두 새 IP입니다. synco 두 이름만 기존 IP/TTL 7200초입니다. 권한 서버 한 곳에 14개 AAAA도 직접 질의했고 IPv6 주소 응답은 없었습니다.
- 일반 DNS로 14개 HTTPS의 인증서·호스트명, 최초 HTTP 상태와 주소 이동을 변경 전 기준선과 비교하여 전부 일치했습니다. 이동을 따라간 최종 응답도 전부 HTTP 200입니다. 별도로 새 IP를 지정한 14개 HTTPS도 같은 기준선이 통과했습니다. 강제 IP 시험을 일반 DNS의 전파 완료 증거로 사용하지 않습니다.
- Windows 일반 HTTPS의 첫 입구는 새 IP 10개, 캐시가 남은 `www.coconut.ai.kr`/`www.ziin.site`의 옛 입구 2개, DNS 원본이 미변경인 synco 옛 입구 2개입니다. Windows DNS 캐시에서 앞의 두 www의 옛 IP와 남은 TTL 약 50분을 직접 확인했습니다. 기존 입구의 새 main 전달이 유지되어 모두 정상입니다. synco는 rogeon으로 이동한 뒤 새 main의 로그인 화면에서 HTTP 200입니다.
- 공용 DNS도 실제 wire 조회했습니다. main에서 Cloudflare `1.1.1.1`은 새 IP 12개이며 Google `8.8.8.8`은 rogeon의 옛 캐시가 추가로 남아 새 IP 11개였습니다. 서버·지역·조회 시각별 결과이므로 모든 사용자에 대한 전파 완료를 주장하지 않습니다. `synco.kr` 두 권한 응답의 옛 IP는 이 캐시 사례와 구별합니다.
- main 공식 `live-handoff-main.py verify`가 단일 쓰기 정본, 제품 runtime 5개, PG/MySQL 고유 ID·정본 표시, CEO Loan 공식 경로의 원본 bytes를 다시 확인했습니다. 정본 표시 SHA는 16절과 같은 `3084e58c675afe1db2df7c7b22b3b77338335908e2d4a73ca8c520a2e5462302`입니다. 네 Source의 제품별 공식 `verify-new`도 옛 writer 정지, DB 호환 주소의 새 정본 고유 ID와 GBrain HTTP 200을 확인했습니다. 운영 DB/서비스/예약 작업 설정 및 유료 생성·실제 고객 발송 시험은 실행하지 않았습니다.

**주인님께 남은 조치:** 가비아 `synco.kr` DNS 설정에서 기본 주소와 `www`의 A 연결이 모두 `49.247.192.127`인지 확인하고 저장합니다. 확인된 원본 응답이 기존 IP이므로 단순 캐시 대기로 전체 완료 처리하지 않습니다. 저장 후 같은 권한 DNS·실제 HTTPS 기준으로 다시 확인합니다.

**보존할 운영 조건:** 옛 서버는 공개 캐시 전달 외에도 기존 SQL/CLI/자료 주소의 호환 전달, 별도 장애 영역의 암호화 백업·복구 키와 이전 대상 밖의 서비스에 사용됩니다. DNS 변경만으로 옛 서버 해지/삭제가 안전해지지 않습니다. 서버 삭제와 vdb 포맷은 별도 지시·잔여 의존성 정리가 필요합니다.

현재 확인 근거는 같은 작업 outputs의 `운영서버_통합_DNS확인_20260918.json`이며 원본 DNS wire/HTTPS/정본 검증을 포함합니다. 1~16절의 당시 DNS 상태와 승인 기록은 과거 이력으로 보존하고, 현재 DNS 상태는 이 17절을 우선합니다.


## 18. 2026-09-18 00:35 KST DNS 14개 완료·남은 운영 후속 작업

**Coconut/FundKeeper·RNDLOG·CEO Loan·ZiiN·GBrain의 코드·DB·자료·쓰기·예약 책임 이전과 최종 백업 독립 복원은 16절에서 완료했고, 공개 DNS 14개 연결까지 완료됐습니다.** 주인님의 마지막 synco 기본/www 변경 뒤 가비아 권한 DNS 서버 3곳에 직접 조회했습니다. 00:35 KST 총 42개 A의 권한 응답이 모두 main `49.247.192.127`입니다. main에서 Google·Cloudflare 각각 14개도 같은 IP이고, Windows 일반 HTTPS 14개의 인증서·첫 응답·주소 이동·최종 200도 정상입니다. 17절의 synco 미변경은 해소됐으며 당시 증거는 이력으로 보존합니다.

모든 지역의 DNS 캐시가 사라졌다는 뜻은 아닙니다. Windows의 www.coconut/www.ziin은 옛 주소 캐시로 기존 입구에 접속하고, 그 입구가 main으로 정상 전달합니다. main 공식 verify는 단일 쓰기 정본, 제품 서비스 5개, 정본 표시·DB 고유 ID와 CEO Loan 공식 자료의 원본 내용을 다시 확인했습니다. 정상 예약 13개는 활성 상태입니다. CEO Loan/RNDLOG의 ceos는 00:05, delivery는 00:25, sms는 00:30에 각각 실행되어 실제 시작·종료 시각과 정상 종료를 확인했습니다. 아직 실행되지 않은 서비스의 기본 success 표시를 실제 실행 성공으로 판정하지 않으며, 고객 수령·대외 처리 건수까지 확인한 증거로 확대하지 않습니다.

### 정상 일정에서 확인할 결과

| 일정(KST) | 확인할 실제 결과 | 00:35 확인 상태 |
|---|---|---|
| 9월 18일 02:10 | Coconut 기존 캐시 생성 | 예약 활성, 첫 실제 실행 전 |
| 9월 18일 03:22:43 | 중앙 인증서 갱신 점검과 필요한 전달 | 예약 활성, 첫 실제 실행 전. 기존 CA 갱신 경로 준비시험 근거 유지 |
| 9월 18일 03:30 | 조정실 새 Codex 대화만 GBrain 기억 정리 | Windows 작업 활성·준비 상태, 첫 실제 실행 전 |
| 9월 18일 03:40 | 회사 DB 백업의 완료 파일·보존·복구 가능성 | 예약 활성, 첫 실제 실행 전 |
| 9월 18일 08:30/09:10/09:30 | Coconut 원래 token/accounts/prices 작업 | 예약 활성, 첫 실제 실행 전 |
| 9월 21일 05:26:32 | ZiiN 주간 Kakao 원래 작업 | 기존 마지막 실행 기록 보존, 다음 일정 대기 |

Coconut의 실제 관리자 세션 보존은 기존 검증에서 재사용할 유효 세션이 없어 미확인으로 남았습니다. 이번 DNS 확인은 인증 후 계정 동작의 새 증거가 아닙니다. 격리 로그인과 허용된 내부 AI 1회 응답의 증거는 유지하고 추가 유료 생성·고객 발송·주문 시험은 실행하지 않습니다.

### 옛 서버 의존성·백업·저장공간 후속

1. **옛 주소와 제외 서비스 정리:** 기존 GBrain SSH CLI, SQL·자료 게이트웨이, DNS 캐시의 옛 HTTPS 입구와 rn.studio/office.exdigm.com의 일반 Certbot, 개인 Hermes·Portainer 및 보관 자료가 옛 서버에 남아 있습니다. 필요한 소비자를 main의 직접 주소·경로로 옮기고 서버별 잔여 역할을 확인해야 합니다. Windows 기억 정리와 제품 자료 접근의 옛 주소 사용 여부도 포함합니다. 해지 전에 옛 Codex 대화·결과의 보관 위치와 다시 읽을 수 있는지도 확인합니다. 옛 쓰기 정본을 다시 열지는 않습니다.
2. **지속적인 서버 외부 백업:** 전환 시점의 암호화 묶음과 복구 키는 옛 DB의 별도 디스크에 보관하고 독립 복원을 확인했습니다. 매일 03:40 작업은 company_main을 새 main의 `/mnt/ceo-loan-backups`에 기본 14일 보관합니다. 해당 서비스와 스크립트의 경로에는 서버 외부 전달 명령이 없습니다. 전환 뒤 쌓이는 필요한 DB·파일의 정기 외부 보관, 보존 기간, 복구 검증 범위와 장소는 후속 결정입니다. 옛 DB 해지 전 백업과 복구 키의 별도 보관·독립 복원도 확인해야 합니다. 정기 외부 백업의 갱신 완료를 주장하지 않습니다.
3. **저장공간 배치:** main 주 디스크 여유는 24,750,563,328 bytes(약 23.05GiB), 기존 최소 여유는 20GiB입니다. 외부 백업 디스크 여유는 11,570,393,088 bytes(약 10.78GiB), 최소 여유는 10GiB입니다. 추가 vdb 200GiB는 파일시스템과 마운트가 없습니다. 검증 복원본·이미지·현재 데이터·암호화 백업의 보관 책임을 확인한 뒤 이번 작업 소유의 불필요한 복사본 정리와 추가 디스크 사용을 정합니다. 포맷·마운트·데이터 이동은 변경 범위·복구·검증 방법을 구체화하고 별도 지시 후 실행합니다. 이번에는 변경하지 않았습니다.
...
- FundKeeper 소스는 `b5b601d5231271d9bdb881739b2eebabb13dda8e` 두 파일이며 origin/master의 실제 HEAD를 확인했다. Main infra는 `0a6e400240efd525b6c017fd071c6c5776889d12` 다섯 파일이며 기존 Git metadata 소유·권한을 보존했다. 조정실 코드·카드·네 AGENTS와 이 절의 구현 기록은 `e207fadc44bcd89e616d18c68952ca960dd2c4ab` 여덟 파일로 origin/main에 저장했다. 이 아래 영수증 문구를 추가한 문서 저장은 코드·실행 설정을 바꾸지 않는다. 병렬 사용자의 controlroom-unification/Exdigm 문서는 이번 저장 대상에 포함하지 않는다.
- 최신 설정 복구 암호문은 `checkpoint-base-20260918T055421Z-e4f70aa2.tar.gz.gpg`, 189,506bytes, SHA-256 `5abda5bf54291520a18d0318875fbe0967cdb79ff27a135a065557e26ab03d6c`다. 별도 DB 서버 `/mnt/consolidation-20260916/offsite`의 실제 원본에서 GPG 무결성·24파일 SHA·infra bundle clone/HEAD·두 캐시/두 Windows 기억 스크립트 SHA와 compile·백업3일/캐시 override·fstab/권한 metadata를 확인하고 임시 평문을 정리했다. Main 영수증은 `/srv/consolidation/work/followup-config-recovery-20260918/result.json`, 별도 서버 증거는 `/mnt/consolidation-20260916/followup-config-recovery-20260918-proof.json`이다.
- 캐시 이미지 복구 추가 암호문은 `checkpoint-base-20260918T060108Z-0a12b715.tar.gz.gpg`, 177,080bytes, SHA-256 `44125c255619107f9577bd87f73389684dcc18e4568facab45a7304e8ecc88b4`다. 같은 별도 보관 위치에서 GPG·14파일 manifest·infra bundle을 검증했다. 이 묶음의 `cache-image.oci.tar`는40,960bytes의 추가 레이어/metadata이며 기존17개 base 레이어를 포함한 단독 전체 이미지가 아니다. 원래 cold runtime 복구 묶음에서 frozen base `sha256:f5501c4e097aac61aaec79235a29cb04d9445f51b6e0e731218c3680a2e189cf`를 먼저 복원한 뒤 이 추가 묶음을 native `docker image load`로 읽는다.
- 15:06 KST에는 별도 서버의 실제 암호문을 복호화해40KB 이미지를 SSH 표준입력으로 격리 엔진에 직접 전달했다. Windows 디스크에 이미지 파일을 저장하지 않았다. 시험 환경의 새 이미지가 없는 상태에서 로드해 최종 이미지 ID `sha256:1bfa57ef77b51e630c75b86313586e1551676f63a2aec4b465e731fbad2d1ae9`와 두 소스 SHA가 현재 운영 예약과 정확히 일치했다. 공개 네트워크/포트 없는 읽기 시험 후 이번 컨테이너와 전용 엔진 프로세스는 정리했고 원래 네 시험 컨테이너 및 운영 Docker 엔진은 보존했다. Main 증거는 `/srv/consolidation/infra/validation-coconut-cache-offsite-native-restore-20260918.json`, 별도 서버 증거는 `/mnt/consolidation-20260916/cache-runtime-source-native-restore-20260918-proof.json`이다. 기존 전체 cold DB/data/images 및 MySQL companion·code/access 복구 묶음도 계속 필요하다.
