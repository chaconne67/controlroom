...
- 기획·리서치·작업 계획의 파일 원본은 `~/projects/_control-docs`의 비공개 Git에서 관리하고 각 프로젝트 `docs`로 연결한다. `~`는 Windows의 `USERPROFILE`, macOS·Linux의 `HOME`이며 등록된 프로젝트 경로를 우선한다. 코드·테스트가 읽는 자료와 코드와 함께 바뀌는 기술 계약은 서버 코드 저장소에 둔다. 공통 지침·스킬은 현재 조정실의 kmh-agent-kit가 원본이다.
...
- `kitpull`로 키트·기획 문서·Venture의 커밋을 받은 뒤 프로젝트 `docs/README.md`의 진행 중 작업과 해당 계획의 재개 정보를 확인한다.
