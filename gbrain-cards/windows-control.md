...
- 실제 운영 인계는 2026-09-17 22:30~22:33 KST, 전체 172.469초에 복구 절차까지 끝났다. 2026-09-18 00:14 KST 공개 DNS 원본 14개 이름 중 12개는 main을 가리키며 synco.kr/www 두 이름은 기존 49.247.205.170을 반환한다. DNS 캐시의 옛 공개 입구와 기존 GBrain CLI/SQL/자료 호환 주소는 main으로 전달한다. 현행 코드·자료·실행 책임과 검증·복구 정보는 `~/controlroom/docs/production-server-consolidation-20260916.md` 최신 절과 프로젝트 AGENTS.md를 우선한다. 옛 서버 writer는 정지·읽기 전용이며 재개하거나 그곳에 배포하지 않는다. 주인님이 가비아 DNS를 수정했고 synco 기본/www 두 A 레코드의 변경 확인이 남았다. 옛 서버는 호환 전달·별도 복구 백업과 제외 서비스를 맡으므로 삭제/해지·vdb 포맷은 별도 지시와 잔여 의존성 정리가 필요하다.
