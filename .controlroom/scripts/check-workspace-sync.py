...
class WorkspaceSyncTests(unittest.TestCase):
    ...
    def test_server_workspace_override_keeps_operating_repositories(self):
        ...
        self.assertEqual(Path(run('git', 'rev-parse', '--show-toplevel', cwd=workspace / 'venture').stdout.strip()).resolve(), workspace.resolve())
    ...
    def test_nested_venture_git_is_archived_and_single_git_remains(self):
        ...
        self.assertEqual(Path(run('git', 'rev-parse', '--show-toplevel', cwd=code).stdout.strip()).resolve(), repo.resolve())
...
class MainServerTests(unittest.TestCase):
    ...
    def test_main_server_install_pull_verify_push_preserve_product_repositories(self):
        ...
        self.assertEqual(Path(run('git', 'rev-parse', '--show-toplevel', cwd=source / 'venture').stdout.strip()).resolve(), source.resolve())
