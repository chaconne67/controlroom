...
- 사용자가 대화하는 조정실은 로컬 데스크탑이다. main(`49.247.192.127`)에는 PC가 꺼져 있을 때도 승인된 자동 수정 작업을 수행하는 서버 조정실을 둔다. 두 조정실은 같은 controlroom Git의 지침·스킬·계획과 공용 GBrain을 사용한다. main의 실제 조정실 Git 루트는 `~/controlroom-workspaces`이며 운영 코드 루트 `~/projects`와 분리한다. 프로젝트별 등록·연결 폴더를 사용하지 않는다.
...
- 기획·리서치·작업 계획의 파일 원본은 현재 조정실 루트의 `<프로젝트>/docs` 실제 폴더에서 통합 controlroom Git으로 관리한다. PC·노트북 루트는 `~/projects`, main 조정실 루트는 `~/controlroom-workspaces`이며 공통 도구는 그 안의 `.controlroom`이다. GitHub와 각 조정실은 같은 상대 구조를 사용한다. `~`는 Windows의 `USERPROFILE`, macOS·Linux의 `HOME`이다. 코드·테스트가 읽는 자료와 코드와 함께 바뀌는 기술 계약은 서버 코드 저장소에 둔다. 공통 지침·스킬과 프로젝트 기획의 원본은 현재 조정실의 controlroom 저장소다.
...
- 조정실: `C:\Users\chaconne\projects\fundkeeper`, `rndlog`, `ceoloan`, `ziin`. PC·노트북은 기본 `~/projects/<프로젝트>`, main 조정실은 `~/controlroom-workspaces/<프로젝트>`를 사용한다.
...
- Exdigm은 전용 서버의 기존 실제 저장소·개발 서버를 유지하며 main 통합 프로젝트로 옮기지 않는다. Venture는 현재 조정실 루트의 실제 `venture` 폴더에서 실행한다.
