...
- 실제 운영 인계는 2026-09-17 22:30~22:33 KST, 전체 172.469초에 복구 절차까지 끝났다. 2026-09-18 00:35 KST 가비아 권한 DNS 3곳과 Google/Cloudflare 각각에서 공개 DNS 14개 이름 모두 main을 가리킴을 확인했다. 당시 DNS 캐시의 옛 공개 입구와 GBrain CLI/SQL/자료 호환 주소는 main으로 전달했다. 현행 GBrain 접속은 아래 공용 GBrain 실행 절을 따른다. 현행 코드·자료·실행 책임과 검증·복구 정보는 현재 조정실 루트의 `.controlroom/docs/production-server-consolidation-20260916.md` 최신 절과 프로젝트 AGENTS.md를 우선한다. 옛 서버 writer는 정지·읽기 전용이며 재개하거나 그곳에 배포하지 않는다. 주인님의 가비아 DNS 수정은 완료됐다. 실제 저장공간·일일 전체 DB 복원 결과는 통합 문서20절, 정상 예약·인증서 전달의 당시 결과는21절, 기억8개 저장·Coconut 캐시 마감과백업3일 보관은22절, 옛 서버별 호환 주소·지속 외부 백업·해지 후속은18절을 따른다. 옛 서버는 호환 전달·별도 복구 백업과 제외 서비스를 맡으므로 옛 서버 삭제/해지는 별도 지시와 잔여 의존성 정리가 필요하다. 추가 vdb 저장 배치는 2026-09-18 후속 지시에 따라 통합 문서20절에서 적용했다.
- 사용자가 대화하는 조정실은 로컬 데스크탑이다. main(`49.247.192.127`)에는 PC가 꺼져 있을 때도 승인된 자동 수정 작업을 수행하는 서버 조정실을 둔다. 두 조정실은 같은 controlroom Git의 지침·스킬·계획과 공용 GBrain을 사용한다. `--main-server` 설치는 기존 `~/projects/<프로젝트>` 코드 저장소에 지침·스킬만 추가하고, 공통 원본 Git은 `~/.local/share/controlroom/source`에 보관한다. 기존 코드와 Git·문서·원본 스킬은 보존한다. 이 옵션을 아직 실행하지 않았다면 현재 폴더를 설치 완료 상태로 간주하지 않는다.
...
- 기획·리서치·작업 계획의 공유 원본은 Controlroom Git의 `<프로젝트>/docs`다. PC·노트북의 원본 루트는 `~/projects`, main 서버 모드의 원본 루트는 `~/.local/share/controlroom/source`이며 공통 도구는 그 안의 `.controlroom`이다. main의 기존 제품 `docs`는 제품 저장소 소유이며 공유 기획 원본으로 교체하지 않는다. `~`는 Windows의 `USERPROFILE`, macOS·Linux의 `HOME`이다. 코드·테스트가 읽는 자료와 코드와 함께 바뀌는 기술 계약은 서버 코드 저장소에 둔다.
...
- 조정실: `C:\Users\chaconne\projects\fundkeeper`, `rndlog`, `ceoloan`, `ziin`. PC·노트북은 기본 `~/projects/<프로젝트>`를 사용한다. main 서버 모드는 같은 이름의 기존 코드 폴더에서 실행하며, 다른 프로젝트 루트를 지정한 설치는 저장된 `--workspace` 경로를 따른다.
...
- Exdigm은 전용 서버의 기존 실제 저장소·개발 서버를 유지하며 main 통합 프로젝트로 옮기지 않는다. Venture는 PC·노트북 조정실 또는 main의 기존 프로젝트 루트에 있는 실제 `venture` 코드 폴더에서 실행한다. 공통 원본 보관 폴더에는 Venture 코드를 복제하지 않는다.
- 현행 구조·접속·복구 안내는 통합 문서 19절을 우선하며 앞 절의 당시 경로는 이력이다. 조정실의 공통 지침·프로젝트 기획 원본과 실제 설치 구조는 controlroom 저장소에서 관리한다.

## 작업 이어받기

- `controlroom pull` 후 Controlroom 원본의 프로젝트 `docs/README.md`에서 진행 중 작업과 재개 정보를 확인한다. 일반 설치는 Venture의 커밋도 받지만 main 서버 모드의 pull·push는 공통 원본과 배치 지침·스킬만 다루며 제품 코드 Git을 동기화하지 않는다.
