...
- 기획·리서치·작업 계획의 파일 원본은 `~/projects/<프로젝트>/docs`의 실제 폴더에서 통합 controlroom Git으로 관리한다. 공통 도구는 `~/projects/.controlroom`이며 GitHub와 각 조정실은 같은 상대 구조를 사용한다. `~`는 Windows의 `USERPROFILE`, macOS·Linux의 `HOME`이다. 코드·테스트가 읽는 자료와 코드와 함께 바뀌는 기술 계약은 서버 코드 저장소에 둔다. 공통 지침·스킬과 프로젝트 기획의 원본은 현재 조정실의 controlroom 저장소다.
