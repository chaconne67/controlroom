...
- `kitpull` 후 Controlroom 원본의 프로젝트 `docs/README.md`에서 진행 중 작업과 재개 정보를 확인한다. 일반 설치는 Venture의 커밋도 받지만 main 서버 모드의 pull·push는 공통 원본과 배치 지침·스킬만 다루며 제품 코드 Git을 동기화하지 않는다.
...
- 장비를 옮기거나 작업을 마칠 때 기획·재개 정보의 변경을 검토하고 `kitpush`로 저장한다. Venture 코드는 별도 검증·커밋을 마친 것만 전송하며 미커밋 코드를 자동으로 포함하지 않는다.
