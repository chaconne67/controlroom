...
Codex는 조정실의 현재 등록된 Exdigm 프로젝트 폴더에서 실행한다. 현재 경로는 `C:\Users\chaconne\projects\exdigm`이다. 같은 사용자 환경의 Codex 설정·인증·전역 지침·프로젝트 지침·공용 및 프로젝트 스킬을 재사용한다. 에이전트를 서버에 설치하거나 원격에서 실행하지 않는다. 서버의 `/home/chaconne/exdigm-debug` 코드는 SSH로 수정·검증한다.
...
실행기는 대상 선택, 독점 처리, Codex 프로세스 시작, 결과 형식 검사, 제한된 DB 상태 기록을 맡는다. 원인과 해결책의 의미 판단은 Codex에 맡긴다. 기존 수집기가 서버에서 직접 Codex를 시작하지 않는다.
...
- 현재 미정: 자동 운영 배포까지 허용할 정책 범위, 외부 에이전트의 실제 읽기·승인 반환 경로, 조정실의 반복 실행 주체와 예산, 실제 CLI 권한 제한 방식. 각 조건에 의존하지 않는 분류·기록 구현은 먼저 진행할 수 있다.
