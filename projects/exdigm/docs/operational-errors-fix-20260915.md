...
## 현재 재개 기준 — 합동 배포 완료, 사람인·인크루트 추가 보완 배포 대기
...
- 인크루트 근무지역 UI 대응3파일은 df7d5c8a85b3316fcb015f282690c935a2c4b86a로 별도 커밋했다. 부모는 사람인 보완 fa524969이며 debug worktree는 clean이다. 현재 버튼·선택 칩·실제 전송 코드를 한 경로에서 확인하고, 예전 rgn_cd/rgn_det 경로는 제거했다.
- 다음: 운영 반영을 별도 확정한 뒤 공식 prod로 fa524969와 df7d5c8a를 순서대로 반영한다. Git/실행 판본과 서비스 정상 확인 후 사람인 첫 장부 재처리와 인크루트 dry 검증을 각각 수행한다. 인크루트 실게시는 기존 성공 사이트를 재실행하지 않고 실패 사이트만 대상으로 별도 확정한다.
...
- 운영 범위는 미검증이다. 운영과 origin/main은 d5d7ad78이며 df7d5c8a는 push·배포하지 않았다.
- 18:03 이후 새 인크루트 실행은 없다. 실제 로그인 dry와 실패 사이트 실게시 재시도는 운영 반영 및 외부 효과의 별도 확정 뒤 수행한다.
