...
- 작업 위치는 이 프로필을 연결한 현재 프로젝트 폴더이며 기본값은 `~/projects/exdigm`입니다. `~`는 Windows의 `USERPROFILE`, macOS·Linux의 `HOME`입니다. 이미 등록된 사용자 지정 경로를 우선합니다.
...
- 기획·리서치·작업 계획은 `docs/README.md`에서 찾습니다. `docs`는 통합 controlroom 저장소의 `projects/exdigm/docs`에 연결되며 기본 정본은 `~/controlroom/projects/exdigm/docs`입니다.
...
- Exdigm 앱 서버에는 Codex·Claude 개발 에이전트와 조정실 지침·스킬·GBrain 설정을 설치하지 않습니다. main의 자동 수정용 조정실은 같은 controlroom 저장소를 설치·동기화하며, 설치된 프로젝트 경로 등록값을 따릅니다.
