...
작성일: 2026-09-17. 현재 상태: **누적 원본36개/고유물건32개 보관 / 모델v2.1·물리계약2.1.1 / 중앙32테이블에 검증 원문7개·판독8개 검토 대기 적재 / 인증 원본 다운로드 웹 배포·실제HTTPS 확인 / 자동 의미 정확성 미통과**.

추가 검토: 10건은 일반 건물에 집중된 모델 탐색 표본이다. 추가 표본의 필요성·권고 범위·종료 기준은10절, 주인님의 진행 승인과 실제 실행 기록은11절에 기록한다. **4절은 추가 원문·보류 표본을 반영한 최종 모델 v2.1이며,12·13절은 변경 근거와 검증 한계다. 후속 구현·중앙 적재·원본 조회 배포와 정확성 한계는14절에 있다.**

주인님 요청에 따라 CRETOP 상세정보가 저장된 모기지 대상 회사에서 무작위 후보를 뽑아 실제 IROS 등기를 열람했다. 원본과 분석용 구조화 자료는 별도 마운트 디스크에 보관했다. 1~13절은 최초10개와 추가20물건의 설계 조사 종료 시점 기록이다. 후속14절에서 공유 schema/권한과 검증 원문의 검토 대기 적재, 인증 원본 다운로드를 구현·배포했다. 자동 판독을 분석용으로 확정한 것은 아니다.
...
- 기존/작업중 판독의 이름·날짜·금액 오타를 원문 대조로 정정 관측에 기록했다. L02 은행명은 후속 재검증에서 텍스트층과 화면 모두 신한은행임을 확인했으며, 이전 수작업 기준의 조흥은행이 잘못된 값이다. 텍스트층/화면 불일치의 증거로 쓰지 않는다. 이전10건·모델 동결15건은 보존한다.
- 실제 영수증 승인 시각과 신청목록/재열람 표시시각을 구분했다. 저장 실패4건은 공식 view-only로 재개했고 재결제하지 않았다.

12절 조사 종료 시점에는 운영 자동 의미 추출기·중앙DB 설치/역할/적재·인증 다운로드를 구현하지 않았다. 이후14절의 후속 구현·실행 결과를 현재 상태의 기준으로 읽는다. 필드 단위 의미 좌표 정확성은 여전히 승인하지 않았다. 조사 결과의 완성을 운영 자동 적재 경로의 완성으로 판정하지 않는다.
...
## 14. 자동 판독·중앙 적재·원본 조회 연결 (2026-09-17)

주인님의 v2.1 후속 제안에 대한 "다음 진행"으로 원본 판독, 중앙 적재, 인증 원본 조회를 구현하고 실제 경로를 검증했다. **저장·권한·원본 전송은 검증됐지만 자동 의미 판독의 정확성은 승인하지 않았다.** 새로운 독립 원문 N01에서 누락이 확인되어 분석용 확정을 보류한다. 일반 무작위20건을 반복 수집하지 않았다.

### 기준선·승인·보호 조건

- 시작 코드: chaconne@49.247.205.170:/home/chaconne/ceoloan/repo, main 864420e0e91571477950cac50c06be11ecf91c13, clean. 기존 IROS 검사101개가 통과했다.
- 시작 중앙 DB: 49.247.45.243 company_main. 기존 company/ceoloan은 존재하고 real_estate/전용 역할은 없었다. ceoloan은 company.companies 읽기만, company_collector는 기존 회사 수집 쓰기 권한을 보유했다.
- 기존 회사·영업 데이터, 회사 쓰기 권한, 결제/재열람 공식 경로와 동결 조사 자료를 보존했다. 초기 구현 승인에는 운영 웹 배포가 없었다. 이후 주인님이 "원본 다운로드 기능 배포"를 명시 승인해 해당 기능을 배포했다. 예약 배치·문자 발송은 실행하지 않았다.
- 최소 구현 게이트2단계에서 멈췄다. common/llm.py, 설치된 google-genai/pypdf/psycopg, Django 인증/FileResponse를 재사용했다. scripts/common/mortgage/deploy 검색상 공유 등기 적재·인증 등기 다운로드는 없었다. RNDLOG 고객 작업공간 gateway의 권한 경계를 넓히지 않았다. 새 Python 의존성은 없고 웹 전용 전송에 필요한 openssh-client만 Dockerfile에 추가했다.
- 변경 전후 기준선은 공식 IROS/registry 검사, Django check, Compose config, diff check다. 중앙 적재는 기존 company/ceoloan 쓰기 권한 전후 대조 및 별도 검증 DB의 transaction/권한 시험 후 실행했다.

### 공식 경로와 구현 책임

원본 PDF + 동일 원본의 Poppler144dpi 페이지 PNG + native text → 제품 Gemini 의미 판독 → 32테이블 필드 계약·같은 판독 참조 검사 → 원본/JSON 해시와 실제 보관 바이트 확인 → 전용 collector의 한 PostgreSQL transaction → 불변 판독 review_required → 원문 검토 후 published_extraction 선택 → 회사 권한 검사 → 원본 해시 재확인 후 다운로드.

- registry/contract.py가32테이블 물리 열/타입/선택지/참조의 정본이다. 같은 계약에서 생성한 SQL과 deploy/real_estate_v2_1.sql의 일치를 검사한다. 공유 DDL은 중앙 소유자의 독립 SQL이며 Django migration으로 company/cretop/real_estate를 생성·변경하지 않는다.
- registry/extract.py가 의미 판독, registry/repository.py가 원자적·불변 적재와 명시적 확정, scripts/iros_pipeline.py가 extract/ingest/publish/link-collateral의 공식 진입점이다. 브라우저/결제는 기존 iros_dom/iros_ssp의 완료 원문을 재사용한다.
- 프로그램은 파일·페이지·해시·숫자/날짜/선택지/FK·UUID 연결을 검사한다. 소유·채무·말소 범위·사건 관계를 정규식으로 보정하지 않는다. 형식 위반에는 원문/위반 내용을 주어1회 LLM 수정 요청하고, 재실패는 성공/빈 결과로 바꾸지 않는다.
- 최종 연결 버전은 schema2.1.1 / extractor iros-pdf-vision-12 / prompt iros-relations-16이다. 전체26개 판독 테이블을 강하게 제한한 API schema는 실제400 거절을 받았다. 공급자가 수용한3개 근거/항목 typed 변형과 사실 행 envelope를 사용하되 저장 전 모든26개 사실 테이블을 완전한 물리 계약으로 검사한다. 날짜·enum·FK 검사 기준을 낮추지 않았다.
- 이력 누락을 보완하려던 두 단계 실험은 응답 길이/JSON/FK 문제를 해결하지 못했다. 실패 응답/버전은 private 연구 보관본에 남기고, 실패한 별도 실행 분기는 공식 코드에서 제거했다. 최종 단일 경로는 L01 원문으로 형식 통과를 확인했으나 공동담보 참조 누락 때문에 정확성 승인으로 쓰지 않았다.
- 근거 쪽/PDF-point 좌표를 저장한다. page 정밀도의 bbox 미제공은 원본 전체 쪽 크기로 채우며 field/row 정밀도를 주장하지 않는다.
- actual_outstanding_loan_balance / actual_loan_interest_rate / actual_loan_maturity_date는 등기 최고액·신탁기간에서 만들지 않고 분석 view에서 NULL로 유지한다.

### 실제 중앙 DB와 파일 연결

- 중앙 소유자가 company_main에 real_estate32테이블/5읽기 view를 설치했다. 소유자 real_estate_owner(NOLOGIN), 수집자 real_estate_collector(LOGIN), 검토자 real_estate_reviewer(NOLOGIN)다. 기존 company/ceoloan 스키마·권한은 보존했다.
- collector는 company_main 접속과 real_estate SELECT/INSERT만 가진다. production의 PUBLIC CONNECT 제한으로 최초 접속이 거절됐으며, 해당 역할에 company_main CONNECT만 추가해 공식 명령으로 성공했다. company.companies SELECT/INSERT/UPDATE는 모두 없다.
- ceoloan은 등기 전용5개 view SELECT만 받았다. 등기 사실 INSERT나 회사 쓰기 권한은 없다. collector는 분석 확정이나 문서 간 공동담보 동일성 연결을 할 수 없다.
- 원문7개/고유물건7개, 판독 버전8개, 파일 메타데이터15개(PDF7+JSON8), 회사 연결7개/원천회사6개, 이번 실제 열람·결제 기록2개/1,400원을 공식 ingest 명령으로 적재했다. **이전 전체32물건이 중앙 DB에 모두 적재된 것은 아니다.** 검증 대상7원문의8판독과 실제 연결을 먼저 확인했다.
- 각 판독을 재적재해 같은 ID/신규 중복0을 확인했다. 같은 문서/모델/스키마/프롬프트 버전의 내용 변경은 거절하며, 버전이 다르면 원문 하나에 별도 판독을 보존한다.
- 판독8개는 모두 review_required이다. 원문 대조 실패/미완료 사유를 validation_report.source_audit에 package 해시로 묶어 저장했다. published물건0, 현재 소유/권리/신탁 분석 view 모두0건이다. 모델의 complete=true만으로 확정하지 않는다.
- 원본은 /mnt/ceoloan/registry/original/2026/09/<UID>/<PDF SHA256>.pdf, 판독JSON은 structured/2026/09/<UID>/<JSON 파일 SHA256>.json에 저장한다. DB에는 backend/key/sha256/byte_size/mime_type/file UUID와 원본 download_path를 저장한다.
- 중앙 마운트 원본은 누적PDF36개/고유UID32개/60,662,966바이트이며 파일명SHA256과 실제 바이트 전수 대조 오류0이다. 디렉터리0750/파일0640, 기존 /mnt/data MySQL 경계는 유지한다. 별도 백업/물리 복원 검증을 뜻하지 않는다.
- 전송은 중앙의 고정 gateway와 전용 forced SSH 키다. 임의 경로/심볼릭 링크/셸/PTY/포워딩을 허용하지 않는다. 앱 registry-transport는 읽기 키·host pin만 보유하고 DSN은 별도 registry-collector/collector.env(0700/0600)에 둬 웹 mount에 노출하지 않는다. 비밀값은 Git/GBrain/연구 archive에 넣지 않는다.
- route /documents/registry/<document_uuid>/original/은 활성 ADMIN 또는 해당 회사가 실제 배정된 TM만 허용한다. 회사 권한을 파일 읽기보다 먼저 검사하고 private/no-store로 반환한다. JSON은 내부 보관용이다.
- 실제 company_main/ceoloan 역할과 실제 활성 관리자 계정으로 공식 Django route를 호출해PDF7개 모두200/원본SHA256·크기 일치를 확인했다. RequestFactory를 통한 실제 DB·SSH 검증이며 공개 HTTPS 로그인/다운로드 증거는 아니다. 배포 후 실제 HTTPS에서7개 원본을 검증하고 link_status를 active_authenticated로 전환했다.

### 새로운 표본과 의미 정확성 판정

동결 extractor7/prompt11/schema2.1.1의 해시·시각을 먼저 남기고 N01/N02 원문을 확보했다. N01은 새로운 독립 회사의 건물, N02는 기존 A02 공동담보목록의 미열람 관련 건물이다. 원문 확인 후 보류 표본은 소진됐으며 후속 버전의 독립 평가 자료로 다시 세지 않는다.

| 자료 | 확인된 문제와 판정 |
|---|---|
| N01 새 독립 건물 | 원문 근저당 설정10개 중 판독 권리1개. 면적20개·목록 구성원·본문 최신 목록 참조 일부도 누락. complete=true/unresolved_annex_count=0이 원문과 맞지 않아 독립 정확성 검증 실패 |
| N02 관련 건물 | 최신19/20번 설정 중심으로 반환해 이전 설정·전세권·말소와 표제 구성 누락. 다른 법원의 목록 번호/같은 금액으로 같은 채무라고 연결하지 않음 |
| U06 건물만 담보 | 전유면적/대지권 일부는 반환했으나1동 층별 면적과 목적 토지 이력 누락. 전체 표제 정확성 미통과 |
| A02 목록/공유 | 3/10+5/10+2/10 중3/10만 반환. 계약 전체65억원·공동담보64.2억원의 범위/반복 사본을 완전하게 구조화한 상태가 아님 |
| A03 공동1순위 신탁 | 기관별 한도 일부를 반환했으나 스캔 계약 조건 trust_terms는0행. selected_fields_complete=true를 승인하지 않음 |
| L01 최종 재시험 | 원문 없는 말소 후 금액0 문제를 피했으나 직접 공동담보 건물 참조는 누락. 형식 통과/정확성 실패를 구분 |
| L02 재대조 | 원문 은행명은 신한은행. 수작업 기준 조흥은행이 오류. 전체 역할 유효기간·관계8항목 검토는 미완료 |
| L10/B04 복잡 이력 | 실험에서JSON/FK/출력 길이 문제가 발생해 정상 적재package 미확보. 자동 성공을 주장하지 않음 |

N01 본문에 참조된 최신 공동담보목록4개는 원문에 포함되지 않았다. 받은 목록5개를 전체 목록 수령으로 보고하지 않는다. N02 관련 토지1개는 미열람이다.

**자동 대량 추출/분석 활성화는 미승인 상태**다. 다음 보완은 원문 항목·권리·목록 구성원의 누락을 판독 단계에서 검출하고, 의미 결과와 DB행·근거 키 생성을 나누는 방법을 검토해야 한다. 이번 실패에 맞춘 수작업 금액/이름 패치를 공식 경로에 넣지 않는다. 그 구조 변경과 새로운 독립 표본 검증은 아직 구현·완료하지 않았다.

### 검증·리뷰·보존

- 기존101+신규37=138검사 통과. Django check/Compose config/diff check/생성DDL 일치를 통과했다.
- 별도 iros_validation_20260917_v3 DB에서 실제 PDF/JSON 해시,7package 적재·중복 방지, 같은 버전 충돌 보존, 부분INSERT 후rollback, 판독 간FK 차단, collector 확정/회사 쓰기 거절, 검토·분석 전환, 실제 원본 전송을12항목으로 검증했다. 시험 reviewed는 전환 fixture이며 의미 정확성 승인이 아니다. production은 모두draft다.
- 고정 base864420e의19파일과 직접 소비자를 root code-review-loop로 재검토하고, 명시 배포 승인 후 기존 컨테이너 보존 옵션을 추가한 deploy.sh까지20파일 범위로 재검토했다. DB8항목 승인 조건 누락, 빈/NULL좌표 허용, 공동담보 연결 승인 조건 누락, 제한된DB collector CONNECT 누락4결함을 직접 재현해 수정했다. 최종 전체 diff의 승인finding0/열린 material contract question0이다.
- 최종 코드는 main efc8f2ddff789084543a6ea7d47f089de8b0ab1d(선행18284d5/97434c3)에 저장하고 GitHub ceoloan/main에 push했다. 작업 트리는clean이다.
- 별도 ceoloan-registry-validation:18284d5 이미지를 빌드했다. Python3.13.15/SSH/32테이블 계약/배포 Django check가 통과했고 .env는 이미지에 없다. digest sha256:cfff666b6a695efc917384c5be13ba4c9d5e9b8bc55775324b3cd508647c4df6. 네트워크 없는 일회 컨테이너 검사 후 종료했으며 운영 서비스로 실행하지 않았다.
- 주인님의 명시 승인 후 공식 ./deploy.sh --no-migrate --keep-orphans로 웹 배포했다. DB migrate plan은 미적용 작업0이며 migration은 실행하지 않았다. .env 기존 바이트를 private 보관하고 REGISTRY_STORAGE_SSH_HOST1개만 추가했다.
- --keep-orphans는 종료된 ceoloan-db와 consolidation-https-rehearsal을 보존하기 위해 기존 deploy.sh의 옵션 처리를 재사용해 추가했다. 기본 배포 동작은 유지한다. bash 문법 전후 검사와 실제 배포 후 두 컨테이너 ID/종료 상태 보존을 확인했다.
- 운영 web 이미지 sha256:a5ceae7de68f56ed255b310439928686968ae8df999f0a24c1d8f560ef0e7a3f, nginx sha256:7d976f5a0fcda15a9566fd5a9464d1bd21c810b8128c28ebdaa37e3510a45985. 실행 web의20파일 SHA256이 검토한 코드와 일치하고 healthy다.
- 공개 HTTPS에 기존 유효 관리자 로그인 세션을 사용해 원본PDF7개200/크기·해시 일치/private cache를 확인했다. 새 세션을 만들거나 비밀번호를 바꾸지 않았다. 비로그인은 로그인302, 실제 회사 배정 없는TM/영업담당자는 각7문서404였다. 배정된TM의 실제 성공은 해당 세션/배정 표본이 없어 이번 HTTPS 증거에 포함하지 않는다.
- 원본7개 링크는 active_authenticated이고 판독8개는 여전히review_required/published0이다. collector DSN은 웹 mount에 없다.
- 이번 유료 원문2개/19쪽/1,400원. 이전 추가24회16,800원과 합쳐 추가26회18,200원 상한을 모두 사용했다. 최초10회 포함36회25,200원. 추가 결제 승인 잔액0이다.
- 이번 두 결제/수령 호출에서 입력 데스크톱Default 전후/도구Chrome 전경 이벤트0/호출후 소유profile PID0을 관측했다. 모니터·영수증·원본·렌더·판독 시도/실패 응답·동결·DB/권한/리뷰 검증 자료는 private 연구 보관본에 보존한다.

새 private 연구 보관본은 /mnt/ceoloan/registry/studies/20260917_auto_validation/이다. 판독 시도/원본·렌더·공식 화면 근거·동결·의미 평가·DB/권한/리뷰/배포 검증428개/40,681,174바이트를 checksum으로 확인했다. DSN/env/SSH키/인증probe는 제외했고 기존 두 연구 보관본을 덮어쓰지 않았다.

### 재개 정보

- 정본은 기존49.247.205.170:/home/chaconne/ceoloan/repo와49.247.45.243 company_main이다. 통합 준비본을 정본으로 사용하지 않는다.
- 현재: 중앙 공유schema/권한/8draft 적재와 인증 원본 조회 웹 배포·실제HTTPS 검증 완료. 자동 의미 정확성 검증은 실패하여 분석 확정0건이다. 기존 원문에서 판독 누락을 보완할 수 있으며 자동 대량 분석 활성화는 아직 승인된 결과가 아니다.
- 재개 시 mainHEAD/GitHub 일치·clean, production draft8/published0, 원본 해시를 확인한다. 실패package를 같은 버전으로 덮어쓰지 말고 기존 원문/시도/비용/미수령 참조를 보존한다.
- 다음 자동 판독 작업은 누락 검출과 의미 결과→DB연결 구조 검증이다. 같은 표본 재시험은 독립 평가로 세지 않는다. 새 유료 원문이 필요하면26회 상한의 추가 승인이 필요하다.
- 오늘22:14 예정 통합 작업에는 추가 shared schema/역할, 새 마운트 연구 보관본, 앱 registry-transport/registry-collector 디렉터리가 인계 자료에 반영되는지 확인해야 한다. forced SSH 키는 old app IP49.247.205.170에만 제한된다. 새 운영IP가 정본으로 선택되면 키/known-host/private 설정 인계와 허용 원천을 통합 범위에서 검증해야 한다. 이 작업은 별도 서버 통합 절차를 변경하거나 새IP를 미리 허용하지 않았다.
